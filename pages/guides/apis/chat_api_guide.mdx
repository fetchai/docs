...
respective purposes.
    ...
    `normalize_question` function converts questions from a JSON response, identified by the key 'text', into lowercase and removes any punctuation. 
    ...
    `send_user_choice_as_uuid` function is designed to send user's choice back to AI-engine through Chat API, specially in case of user getting options from multiple
    ...
    `send_user_message` function to sends a message from the user to the chat session. This is used to submit user input as part of the chat flow.
    ...
    `is_uuid` accepts a key argument, which then verified is the key given is in the correct UUID format. 
    ...
    `stop_session` posts a request to the chat api to stop the session passed.

    <Callout type="warning" emoji="⚠️">
      To get better understanding of the workflow how User, deltaV and AI-engine work collectivly to achieve user's objective please refer [Powering connections and smart operations in DeltaV ↗️](https://fetch.ai/docs/concepts/ai-engine/powering-connections-and-smart-operations-in-deltav).
    </Callout>
...
Chunk-by-chunk explaination:
    ...
    The above section of code creates payload(data) for request to start a session. Please update the following details accordingly:
        ...
        - token: The token you got in section [Prerequisites ↗️](/apis/ai-engine/chat_api_example#prerequisites) and place it in place of YOUR_FAUNA_TOKEN.

...
# uAgents and Functions Creation using APIs

## Introduction

This guide shows how to create **uAgents** and **uAgent Functions** in Agentverse using APIs. With this guide, we set up a Python script that interacts with the Agentverse and help us to create uAgents and uAgent Functions. Before you begin, it is necessary that you satisfy the following requirements:
...
## How to create uAgents and respective Functions
...
This script interacts with the Agentverse API to achieve the uAgent and Function creation tasks.
...
2. Take the uAgent name from user and store the uAgent's address:

    ```python
    # Take name of uAgent from user
    name = input('Please give name of your agent? ')

    # Create payload for uAgent creation request
    agent_creation_data = {
        "name": name
    }

    # Post request to create an uAgent and store address
...
3. Take code from `agent.py` file. Then, store it in a dedicated script for the created uAgent:

    ```python
    # Reading code to be placed in uAgent
    ...
    agent_code_data = {
        ...
        # Creating agent.py script for created uAgent
    ...
    time.sleep(10) # waiting before getting uAgent's protocol
    ```

4. Requesting protocol digest for the created uAgent:

    ```python
    # Request to get uAgent protocol digest
...
5. Request `Model` digest and name using Almanac APIs:
    ...
    ```python
    # Request to get uAgent's model details
...
6. Now, save all the required details to create the uAgent Function. Then, create the uAgent Function on the basis of the details received:
...
6. Provide uAgent and Function details as asked, then, check uAgent and function on Agentverse.

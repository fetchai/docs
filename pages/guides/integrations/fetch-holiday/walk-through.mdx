...
You first need to satisfy the requirements in [Getting Started ↗](/guides/integrations/fetch-holiday/getting-started.mdx) section for this example.
...
These directories will contain the different files you will need to correctly run the project.3
...
2. Within this script, import the necessary modules, create an agent "top_activities" , and make sure it has enough funds to register in the [Almanac contract ↗](/references/contracts/uagents-almanac/almanac-overview.mdx):
...
5. We then need to define how the `top_activities` agent responds when it receives a message related to `TopActivities`. We define how the agent should respond when it gets a message about top travel activities. It takes the user's message, creates a template for the AI, sends that to the AI for a response, turns the AI's answer into a list of options, and sends these options back to the user. If something goes wrong, it lets the user know there was an issue:
    ...
    Within a try-except block, any exceptions that may occur during this process are logged as warnings, and an error response is sent back to the sender. We finally use the `.include()` method to include the `TopActivities` protocol in the agent, ensuring it's ready to handle messages of this type.
...
1. Within the destinations directory previously created, create a Python script for this agent and name it: `touch flights.py`.
...
Now that we have defined our agents, we would need to define the structured data formats that these agents will use to communicate with each other or with external systems. There are 4 structured data models which we need to define within the `messages` directory previously created within `src` folder:

#### Flight

The Flights model is designed to hold flight-related information with various fields, each annotated with descriptions and optional constraints. It's used to standardize and validate data exchanged between agents or systems within the uAgents integration.
...
1. Within the `messages` folder, let's create a Python script and name it: `touch general.py`.
...
1. Within the `messages` folder, let's create a Python script and name it: `touch top_activities.py`.
...
1. Within the `messages` folder, let's create a Python script and name it: `touch top_destinations.py`.
...
3. First of all, we need to import the needed classes. Then, we proceed and define a `OpenAILLM` class. This class encapsulates methods for interacting with OpenAI's large language model.
    ...
    The class provides methods for interacting with OpenAI's LLM. It accepts two arguments during initialization:
...
We are now ready to write the code for our main script. In the `src` folder in `fetch-holiday`, let's create a Python script and name it: `touch main.py`.
...
Copy the `{top_dest_address}` value and paste it somewhere safe. You will need it in the following step.
...
2. You will need to paste the following code in it:
    ...
    ```py copy fielname="top_dest_client.py"

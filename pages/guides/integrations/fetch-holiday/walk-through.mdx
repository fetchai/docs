...
You first need to satisfy the requirements in [Getting Started 🎯↗️](/guides/integrations/fetch-holiday/getting-started) section for this example.
...
2. Paste in the following snippet:
    ...
    We've just imported all the necessary modules, created an agent named `top_activities` using a secret [seed ↗](https://en.wikipedia.org/wiki/Random_seed) , and with ` fund_agent_if_low(agent.wallet.address())` made sure that this agent has enough funds to register in the [Almanac contract ↗️](/references/contracts/uagents-almanac/almanac-overview).

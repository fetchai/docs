...
if you have correctly followed the [installation guide ↗️](/guides/ledger/installation), then you should now have `fetchd` successfully installed in your path.
...
Check the [network page ↗️](/references/ledger/active-networks) for the list of supported versions per network.
...
4. Your local node will now synchronize with the network, replaying all blocks and transactions. This process may take some time depending on factors like the network's age and your disk speed. Consider using [chain snapshots ↗️](../snapshots/) to speed up this process.

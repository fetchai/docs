...
In this guide we will show how to set up protocols and different AI Agents to implement a cleaning service between clients and cleaning service providers using the `uagents` library.
...
2.  Install tortoise orm library using `pip install tortoise-orm` in your terminal. (Tortoise is an easy-to-use Object-Relational Mapping (ORM) library for Python that simplifies database interactions).
3.  We then import the necessary classes from `enum` and `tortoise` libraries, and then define the different data models:
...
2.  Install geopy library using `pip install geopy` in your terminal. (Geopy library gives information about location and to calculate distance between two locations).

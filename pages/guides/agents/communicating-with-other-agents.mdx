...
- Local communication.
- Remote communication via the [Almanac Contract ↗️](/references/contracts/uagents-almanac/almanac-overview.mdx)️, and the [AgentVerse Mailbox Service ↗️](/concepts/agent-services/agent-mail.mdx)️.
...
2. Then, let's import these necessary classes from the uagents library: `Agent`, `Context`, `Bureau`, and `Model` Let's then define the message structure for messages to be exchanged between the uAgents using the class `Model`.

    Then, we would need to create the uAgents, `alice` and `bob`, with name and seed parameters.
...
3. Let's now define alice behaviors. We need to define a function for alice to send messages to bob periodically.
    ...
    We then need to define a `alice_message_handler()` function for alice to manage incoming messages.
...
5. Let's then use the `Bureau` class to create a `Bureau` object. This will allow us to run uAgents together in the same script.
...
Remote uAgents communication is achieved by registering the uAgents into the [Almanac contract ↗️](/references/contracts/uagents-almanac/almanac-overview.md)️ and then querying it to retrieve an HTTP endpoint from the recipient uAgent. Registration in the Almanac requires paying a small fee, so make sure to have enough funds to allow for this.
...
2. Let's now create our uAgent, `alice`, by providing `name`, `seed`, `port`, and vendpoint`. Also, make sure it has enough funds to register in the Almanac contract.
...
uAgents can communicate remotely by also using the [Agentverse ↗️](https://agentverse.ai/)️. The Agentverse is a platform that aims at creating a decentralized network of agents capable of communicating and interacting with each other. Agents in the Agentverse can send and receive messages, execute tasks, and collaborate with other agents to achieve various goals.

In this guide, we want to show how to enable remote communications between uAgents using the [Agentverse Mailbox Service ↗️](/guides/agentverse/utilising-the-mailbox.mdx)️.
...
3. _Now your agent is ready to join the Agentverse!__ We can now register our uAgent, `alice`, by providing `name`, `seed`, and `mailbox`. Make sure your agent has enough funds for this:
    ...
    On the _Fetch.ai testnet_, you can use the `fund_agent_if_low` function. This one checks if the balance of the uAgent’s wallet is below a certain threshold, and if so, sends a transaction to fund the wallet with a specified amount of cryptocurrency.

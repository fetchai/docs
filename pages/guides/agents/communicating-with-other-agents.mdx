...
Remote uAgents communication is achieved by registering the uAgents into the [Almanac contract ↗️](/references/contracts/uagents-almanac/almanac-overview.md)️ and then querying it to retrieve an HTTP endpoint from the recipient μAgent. Registration in the Almanac requires paying a small fee, so make sure to have enough funds to allow for this.
...
2. Let's now create our μAgent, `alice`, by providing `name`, `seed`, `port`, and vendpoint`. Also, make sure it has enough funds to register in the Almanac contract.
...
3. _Now your agent is ready to join the Agentverse!__ We can now register our μAgent, `alice`, by providing `name`, `seed`, and `mailbox`. Make sure your agent has enough funds for this:

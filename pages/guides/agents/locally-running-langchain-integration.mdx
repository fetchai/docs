...
This guide demonstrates how to run an agent on your own hardware or infrastructure, making it accessible over **Agentverse** and **DeltaV** using the [Mailroom ↗️](/guides/agentverse/utilising-the-mailbox). The example uses a locally hosted agent that utilizes LangChain's Wikipedia integration to process requests related to Wikipedia search. To get this agent to be [DeltaV ↗️](/concepts/ai-engine/deltav) accessible, we will also go to [Agentverse ↗️](/guides/services/services) to create a new Agent Function for the agent to then allow this agent to be found in DeltaV.

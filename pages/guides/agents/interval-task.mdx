...
Agents can perform an _interval task_ to periodically perform actions with a specified time interval. An interval task is achieved thanks to an `on_interval()` decorator which periodically repeats a given function for the agent. For instance, an agent could send a message every 2 seconds to another agent.
...
2. Let's then import the necessary classes from `uagents` library, `Agent` and `Context`, and create our agent:

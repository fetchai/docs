...
3. Let's define our μAgents, `alice` and `bob`. Ensure they have enough funds in their wallets to carry out transactions, using the `fund_agent_if_low()` function:

...
The **Local Agent Inspector** is a developer tool on Agentverse that enhances the debugging and monitoring experience for [Agents running on a local infrastructure ↗️](/guides/agents/intermediate/agent-types#local-agents). It allows you to track and understand the real-time behavior of these Agents, whether they are connected to Agentverse or not.

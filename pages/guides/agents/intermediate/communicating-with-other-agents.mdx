...
An agent must register to the  [Almanac contract ↗️](/references/contracts/uagents-almanac/almanac-overview) to communicate, to be searched or found. Agents can query this contract to retrieve an HTTP endpoint for a recipient agent. [Registration in the Almanac ↗️](/guides/agents/register-in-almanac) requires paying a small fee, so make sure to have enough funds to allow for this. You can query the Almanac now, by using the search feature on [Agentverse ↗️](https://agentverse.ai/).

...
# Communicating with other uAgents

## Introduction

Communication is an essential feature within any agents network. Communication allows uAgents to work together, exchange information, and forms an organic marketplace.
...
2. Then, we import  `Agent`, `Context`, `Bureau`, and `Model` from the `uagents` library, and we then define the message structure for messages to be exchanged between the uAgents using the class `Model`:
...
3. Now we create two uAgent instances, `sigmar` and `slaanesh`, with `name` and `seed` parameters:
    ...
    In this example we're running multiple uAgents from one file.
...
6. Let's now define the behavior of our second uAgent, `slaanesh`:
...
7. Let's then use the `Bureau` class to create a `Bureau` object. This will allow us to run uAgents together in the same script:
...
An uAgent must register to the  [Almanac contract ↗️](/references/contracts/uagents-almanac/almanac-overview) to communicate, to search for other uAgents or be found. uAgents can query this contract to retrieve an HTTP endpoint for a recipient uAgent. [Registration in the Almanac ↗️](/guides/agents/register-in-almanac) requires paying a small fee, so make sure to have enough funds to allow for this. You can query the Almanac now, by using the search feature on [Agentverse ↗️](https://agentverse.ai/).

Whenever an uAgent registers in the Almanac, it must specify the service [endpoints ↗️](/references/contracts/uagents-almanac/endpoints) alongside a weight parameter for each endpoint provided. uAgents trying to communicate with your uAgent will choose the service endpoints using a weighted random selection.
...
2. Let's now create our uAgent, `sigmar`, by providing `name`, `seed`, `port`, and `endpoint`. Also, make sure it has enough funds to register in the Almanac contract:
...
4. We then need to define a function for `sigmar` to handle incoming messages from other uAgents:
...
The overall script for sigmar uAgent should be looking as follows:
...
We can now proceed by writing the script for uAgent `slaanesh`.

#### Slaanesh

1. In `remote_agents_slaanesh.py` script, import the necessary classes from the `uagents` and `uagents.setup`. Then, define the message structure for messages to be exchanged between the uAgents using the `Model` class, as well as our second uAgent, `slaanesh`, by providing `name`, `seed`, `port`, and `endpoint`. Make sure it has enough funds to register in the Almanac contract:
...
As we touched on before in [Register in Almanac ↗️](/guides/agents/register-in-almanac), when the uAgent uses `.run()` function this tells the `uagents` library to register the uAgent to the Almanac. It's simple, uAgents initialize themselves, and register to a service which acts as a search engine for uAgents (the Almanac) then, when uAgents receive messages they can respond.

## Conclusion

In this guide, we explored two different methods of communication for v using the `uagents` library:

    - **Local communication**.
    - **Remote communication** via the Almanac Contract.

For _local communication_, we learned how to use the `uagents` library to create two uAgents, `sigmar` and `slaanesh`, and enable them to exchange messages with one another. We defined the message structure using the `Model` class and implemented message handlers for both uAgents. By running the script we observed their real-time message exchange.

Next, we delved into _remote communication_, which facilitates interaction between uAgents through the Almanac Contract. This method requires registering the uAgents in the Almanac Contract and querying for **HTTP endpoints** for communication. By running the scripts separately, we could observe the real-time messages exchange, fostering a decentralized network of interacting uAgents.

With this, we suspect you're ready to start building uAgents, as part of multi-agent system the Almanac allows; awesome. If you want to go further though, take a look at the [message verification ↗️](/guides/agents/message-verification) and [sending tokens ↗️](/guides/agents/send-tokens), after-all you do want to be sure you are speaking to who you think you are, and uAgents getting paid is awesome.

...
# Multi-file uAgent pipeline for AI Engine: Hugging face API to create a multi uAgent pipeline
...
Hugging Face API with the Agentverse uAgent, enabling users to select their preferred **text-classification** model. Users can choose 
...
Make sure you have read the following resources before going on with this guide:
    ...
    - [AgeuAgentnt Functions ↗️](/guides/agents/intermediate/agent-functions)
...
Create news [access token ↗️](https://huggingface.co/settings/tokens) in settings for reading access type. This API key will be used in uAgent scripts to fetch model lists and make query to respective models.

## Setting up the uAgents

For setting up Hugging face function we need to create different uAgents as listed below.

### Hugging Face System uAgent

This uAgent helps users with their queries for a text-classification model of their choosing. The uAgent always prioritizes Hugging Face request secondary function. Below is the script for creating our uAgent on [Agentverse ↗️](https://agentverse.ai/agents) using the **+ New Agent** button.

        ```python copy filename="agent.py"
        # Here we demonstrate how we can create a hugging face system uAgent that is compatible with DeltaV.
        ...
        # Define a model class for the Hugging Face uAgent's expected message format
        class HF(Model):
            response : str # This class has a single attribute 'response' that holds the string response from the subtask.

        # Create a protocol for the Hugging Face (HF) uAgent, specifying its communication protocol
        ...
        # Include the Hugging Face protocol in your uAgent
        agent.include(hf_protocol)
        ```

### Hugging Face Request uAgent

This uAgent helps hugging face system to handle hugging face request to user and calculates the response. The uAgent has two fields i.e. `model_id` (to which query has to be made) and `query` (Question needed to be asked to the model). For `model_id` this task always prioritizes `Model List` secondary function to get list of available model of a specific type.

        ```python copy filename="agent.py"
        # Here we demonstrate how we can create a hugging face request uAgent that is compatible with DeltaV.

        # After running this uAgent, it can be registered to DeltaV on Agentverse Services tab. For registration you will have to use the uAgent's address.
        ...
        # Define a model class for the Hugging Face Request uAgent's expected message format.
        class Search(Model):
            model_id : str
            query : str
        ...
        # Create a protocol for the Hugging Face Request(HF) uAgent, specifying its communication protocol.
        ...
        async def handle_message(ctx: Context, sender: str, msg: Search):
            ...
            # sending response to hugging face uAgent
...
This uAgent helps user to look for specific model with search keyword. The uAgent queries hugging face url to get top 5 downloaded model related to search keyword. The uAgent returns list of models and user can select one they need.

        ```python copy filename="agent.py"
        # Here we demonstrate how we can create a model list uAgent that is compatible with DeltaV
        ...
        # Define a model class for the Model List uAgent's expected message format
        class Search(Model):
            search : str # This is a keyword for which user wants to search model

        # Create a protocol for the Model List uAgent, specifying its communication protocol
        ...
        # Include model_list protocol in uAgent
        agent.include(model_list_protocol, publish_manifest = True)
        ```

## Setting up functions

Go to [Agentverse ↗️](https://agentverse.ai/) and create new function for all three uAgents created [above ↗️](/guides/agents/intermediate/primary-secondary-functions#setting-up-the-agents).

The properties of function for each uAgent are listed below.
...
![](../../../../src/images/guides/agentverse/Hugging-Face-System/Hugging-Face-System-2.png)
    ...
    - **Protocol** and **Data Model** will be automatically populated based on the source code of [Hugging face system uAgent ↗️](/guides/agents/intermediate/primary-secondary-functions#setting-up-the-agents).
...
![](../../../../src/images/guides/agentverse/Hugging-Face-System/Hugging-Face-Request-2.png)
    ...
    - **Protocol** and **Data Model** will be automatically populated based on the source code of [Hugging face system uAgent ↗️](/guides/agents/intermediate/primary-secondary-functions#hugging-face-request-agent).
...
![](../../../../src/images/guides/agentverse/Hugging-Face-System/Model-List-2.png)
    ...
    - **Application**: Secondary function.
    - **Protocol** and **Data Model** will be automatically populated based on the source code of [Hugging face system uAgent ↗️](/guides/agents/intermediate/primary-secondary-functions#model-list-agent)

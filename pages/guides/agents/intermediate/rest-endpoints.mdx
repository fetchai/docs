...
# How to add custom REST endpoints to your uAgent


## Introduction
uAgents now support custom REST endpoints using the `on_rest_get()` and `on_rest_post()` decorators, enabling them to handle HTTP GET and POST requests directly. With this addition, uAgents can define specific routes, create request and response models, and interact seamlessly with REST clients. This feature enhances the flexibility of uAgents, allowing them to communicate with external systems using standard web protocols while ensuring that all responses conform to predefined models.
...
<Callout type="warning" emoji="⚠️">
   Please note that this feature is only available at the 'uAgent level', meaning that you cannot add REST endpoints to uAgents protocols.
</Callout>

## Usage

To use the new REST API feature in your uAgent, follow these steps to define custom `GET` and `POST` routes, run the uAgent, and query the endpoints.
...
Use the `@on_rest_post()` decorator to define a custom POST endpoint. You need to provide both a request model (for input) and a response model (for output).
    ...
    <Callout type="info" emoji="ℹ️">
        For querying the uAgent you have to make sure that:

         - You use the correct REST method ("GET" or "POST")
         - You address the uAgent endpoint together with its route e.g `http://localhost:8000/custom_route`
    </Callout>
...
1. Run the uAgent: `python agent.py`
2. Query the uAgent directly through your predefined interfaces:

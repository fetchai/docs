...
# Make your uAgents AI Engine compatible

 In the previous resource, ([uAgent functions ↗️](/guides/agents/intermediate/agent-functions)) we created a simple uAgent that
 returned a dice roll, however this uAgent was already ready to be callable by [AI Engine ↗️](/concepts/ai-engine/ai-engine-intro). Let's get into how.
...
To enable communication from uAgents to the AI Engine, we import **3 objects** that define the allowed responses:
...
1. `UAgentResponseType()`:
   ...
   - We can use `ERROR` to signify the uAgent has encountered an error:

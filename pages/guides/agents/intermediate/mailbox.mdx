...
Let's start by creating a local agent named `alice`. `alice` responds `hello` to any agent that sends a message to her, providing the message is of type `Message`.

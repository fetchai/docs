...
## Introduction
uAgents can't always be online; internet loss will cut your uAgent off from the network, or perhaps your uAgent is
...
middleman that acts as a mailbox for all communication to your uAgent. Messages are stored within the mailbox and your
uAgents will collect them (calls for these in [`uagents` library ↗️](https://github.com/fetchai/uAgents/blob/55fd0f1bd14d4d5fdaaa3dae82e4d6c6c5b9a3cd/python/src/uagents/mailbox.py#L65)) when they're online
again.
...
Now that you have correctly retrieved your uAgent's address, let's head over to the [Agentverse ↗️](https://agentverse.ai)!

## Create a Mailbox in Agentverse

Make sure that you are signed in to the [Agentverse ↗️](https://agentverse.ai) platform. We will now create a Mailbox for our local agent above. To do so, copy the [local uAgent address ↗️](/guides/agents/getting-started/getting-uagent-address#print-uagent-address) available within your terminal and head over to the **Agentverse: My Agents** tab. Once there, click on the **Local Agents** tab and click on **Connect Local Agent** button.

![](src/images/guides/agentverse/mailbox/agentverse:mailbox/mailbox_1.png)

You will need to paste the **address of the local uAgent** you wish to retrieve and wait for confirmation. _Note that it may take some time in order for your uAgent to be retrieved correctly_. Make sure your uAgent is running on your end otherwise the uAgent won't be retrievable!

![](src/images/guides/agentverse/mailbox/agentverse:mailbox/mailbox_2.png)

Once the uAgent is correctly retrieved, you will have to give it a **name**. When you do so and confirm, you will see a **Mailbox API Key** showing up. Copy and paste it within your local uAgent code by filling up the `AGENT_MAILBOX_KEY` field inline.

![](src/images/guides/agentverse/mailbox/agentverse:mailbox/mailbox_3.png)

## Updating your uAgent
...
## Creating a second uAgent

Let's create and run a second uAgent to message Alice every 3 seconds. You need to provide the `ALICE_ADDRESS`, `name` and `seed` parameters to correctly run this code:
 ...
 **You can now restart your uAgent!**

    <Callout type="warning" emoji="⚠️">
      Remember, each uAgent needs a different dedicated Mailbox!
    </Callout>

Now your uAgent doesn't need to be running all the time as messages will be waiting when it comes back online.

**Great! You correctly enrolled your local uAgent on the Agentverse using the Mailbox feature!**

...
You can view your recently connected local Agent by heading to the **Agentvere My Agents** tab and by clicking on **Local Agents**.
...
Cool! You have correctly set up a mailbox for the Agent. You can additionally add some further details to the local Agent such as the `README.md` file. To do so, create a file in your local Agent folder and name it `README.md`. In it, add all the needed details and save it. Then, add the following bits to your local Agent code:
...
- Alice: 
  ...
  ```
  Your agent's address is: agent1qw8jn3nfl2fyyhe7v4x8pfmsge4hs9zqrqw9eq7h7hluzmd0da8z7j0uacx
  INFO:     [Alice]: Starting agent with address: agent1qw8jn3nfl2fyyhe7v4x8pfmsge4hs9zqrqw9eq7h7hluzmd0da8z7j0uacx
  INFO:     [Alice]: Agent inspector available at https://agentverse.ai/inspect/?uri=http%3A//127.0.0.1%3A8002&address=agent1qw8jn3nfl2fyyhe7v4x8pfmsge4hs9zqrqw9eq7h7hluzmd0da8z7j0uacx
  INFO:     [Alice]: Starting server on http://0.0.0.0:8002 (Press CTRL+C to quit)
  INFO:     [Alice]: Starting mailbox client for https://agentverse.ai

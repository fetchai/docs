...
## Local agent setup
   ...
   ```
    ...
    <Callout type="warning" emoji="⚠️">
      Remember that you need to provide the `AGENT_MAILBOX_KEY`, `SEED_PHRASE`, `name`, `seed`  and `mailbox` parameters to correctly run this example.
    </Callout>
...
Let's update `alice`, alice will now print to console any message it receives, provided the message is of type `Message`.
...
```
    ...
    <Callout type="warning" emoji="⚠️">
      Remember that you need to provide the `ALICE_ADDRESS`, `name` and `seed` parameters to correctly run this example.
    </Callout>

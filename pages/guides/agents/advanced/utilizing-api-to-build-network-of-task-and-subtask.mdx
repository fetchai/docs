...
# Multi-file uAgents pipeline for AI Engine: using News API to build network of functions and secondary functions in Agentverse

## Introduction

This uAgents network helps users to read news of different types including **Categorical**, **Country** based or **Keyword** related news. The system operates based on a multiple layers structure of Primary and Secondary functions, each one of them playing a vital role in delivering personalized news content to users.
...
Make sure you have read the following resources before going on with this guide:
    ...
    - [Multi-file uAgents pipeline for AI Engine: Hugging face API to create a multi uAgents pipeline ↗️](/guides/agents/intermediate/primary-secondary-functions)
...
Refer to the [Agentverse Functions ↗️](/guides/agents/intermediate/agent-functions#primary-and-secondary-functions) for more details on Primary and Secondary functions. Also, have a look at the [Agentverse Functions ↗️](/guides/agentverse/agentverse-functions/field-descriptions-for-deltav) for guidance on fields descriptions of your uAgent Functions!

## News API

Login to [News API ↗️](https://newsapi.org/docs/endpoints/top-headlines) and get your API key by logging in and registering for API. This API key will be used in uAgent scripts to fetch news related to a specific type and subtype.

### News Reading URL System uAgent

This uAgent helps users to read news according to their willingness. This uAgent triggers the news details gathering secondary functions and asks the user for the news type and subtype they want to read today. This uAgent responds to the final result to the [DeltaV ↗️](/concepts/ai-engine/deltav) GUI as well.

    ```python copy filename="agent.py"

    # Here we demonstrate how we can create a news reading system uAgent that is compatible with DeltaV
        
    # After running this uAgent, it can be registered to DeltaV on Agentverse. For registration you will have to use the uAgent's address
...
Now click on the **Run** button in the upper right corner of the editor so that you have your news system uAgent up and running!

### News Reading Details uAgent

This uAgent asks users for the type and subtype of news they want to read today from options like **categorical**, **country** related or **keyword** related and respective subtypes. It then directs the user request to relevant secondary function to generate that type of news title and respond it back to the **News Reading system**.

    ```python copy filename="agent.py"
    # Here we demonstrate how we can create a news details uAgent that is compatible with DeltaV.
    # After running this uAgent, it can be registered to DeltaV on Agentverse. For registration you will have to use the uAgent's address.
...
Remember that you need to provide the `api_key` parameter to correctly run this code. Now click on the **Run** button in the upper right corner of the editor so that you have your news system uAgent up and running!
...
This subtask is triggered if the user wants to read regional or country specific news, it gets country as news subtype **News Reading details Secondary function** and get list of news titles.
    ...
    # Include the Generate Regional News protocol in your uAgent
    agent.include(generate_news_protocol)
    ```
...
This subtask is triggered if the user wants to read keyword specific news, it gets keyword as news subtype **News reading details Secondary function** and get list of news titles.
    ...
    # Include the Generate Keyword News protocol in your uAgent
...
Let's navigate to the [Agentverse ↗️](https://agentverse.ai) to start registering your uAgent Functions.

    <Callout type="info" emoji="ℹ️">
      For further information on uAgent Functions and registration process, see [Register uAgent Functions on the Agentverse ↗️](/guides/agentverse/registering-agent-services) resource.

    </Callout>

Considering the local uAgents in this guide, in order to register **Local uAgents and respective Functions** on the Agentverse, you will first need to log in the [Agentverse ↗️](https://agentverse.ai/) and head over to the **My Agents** tab. Then, click on **Local Agents** tab and click one of the **Connect Local Agent** buttons.

![](src/images/guides/uagent/servicefordungeons_1.png)

You will need to provide the **local uAgent address** and make sure it is running on your terminal as only running uAgents can enroll uAgent Functions on the Agentverse!

![](src/images/guides/uagent/servicefordungeons_2.png)

   <Callout type="warning" emoji="⚠️">
     If you see a message saying **"We couldn't find this uAgent address on Fetch Network"** you will need to wait for some time (around 5 minutes) until the protocols are uploaded successfully on the Fetch Network and your uAgent is retrievable.
   </Callout>

Once your uAgent is retrieved correctly, you will see a box depicting it within the **My Agents** tab. Click on it and head over to the **Deploy** tab and click on **+ New Function**. Here, you can now provide the needed details for your uAgent Function in the dedicated fields. Remember to provide detailed descriptions for what your **uAgent Function** does and the **Fields** for data Models expected. understanding of Functions fields descriptions.

You will need to fill the uAgent Function form out with a description for each _Primary_ and _Secondary_ functions.
...
### News Reading URL System
    ...
    - **Protocol** and **Data Model** will be automatically populated based on the source code of [your news reading system uAgent ↗️](/guides/agents/advanced/utilizing-api-to-build-network-of-task-and-subtask#news-reading-agent).
    - **Field descriptions**:
...
### News Reading Details Secondary function
    ...
    - **Protocol** and **Data Model** will be automatically populated based on the source code of [your news generating uAgent ↗️](/guides/agents/advanced/utilizing-api-to-build-network-of-task-and-subtask#news-generating-agent).
...
### Generate Categorical News 
    ...
    - **Application**: Secondary function.
    - **Protocol** and **Data Model** will be automatically populated based on the source code of [your generate categorical uAgent ↗️](/guides/agents/advanced/utilizing-api-to-build-network-of-task-and-subtask#generate-categorical-news).
    - **Field descriptions**:
...
### Generate Regional News Secondary function
    ...
    - **Protocol** and **Data Model** will be automatically populated based on the source code of [your generate regional uAgent ↗️](/guides/agents/advanced/utilizing-api-to-build-network-of-task-and-subtask#generate-regional-news).
    - **Field descriptions**:
...
### Generate Keyword News Secondary function
    ...
    - **Application**: Secondary function.
    - **Protocol** and **Data Model** will be automatically populated based on the source code of [your generate keyword uAgent ↗️](/guides/agents/advanced/utilizing-api-to-build-network-of-task-and-subtask#generate-keyword-news).
    - **Field descriptions**:
...
As your objective (`I want to read news of my choice`) specified on the previous screen contained words related to the description of your news reading system uAgent, your News Reading URL System function is listed as an option.

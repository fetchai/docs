...
First of all, we need to create 2 Python scripts for our two uAgents using the following commands within your terminal:
...
Let's start with the first uAgent of this example, Bob.
...
1. First of all, we import the required libraries and modules:
    ...
    - `Agent`, `Context`, `Model`: From the `uagents` module, these are used to define the uAgent, its context, and the message model.
    - `get_faucet`: From the `uagents.network` module, this function is used to get a faucet for funding wallets.
    - `get_name_service_contract`: From the `uagents.network` module, this function is used to get the contract for registering uAgent names on the network.
...
3. We then need to initialize the uAgent:
    ...
    We have created an uAgent named `bob-0` with a specified `seed`. The uAgent listens on port `8001` and has an `endpoint` for receiving messages.
...
4. We are now ready to set up the **Wallet**, **Name Service Contract**, and **Faucet**:
    ...
    We first create a wallet from a seed phrase and this wallet is used for interacting with the Fetch.ai blockchain. We then proceed and define the Name Service Contract. It retrieves the name service contract to register the uAgent's name on the blockchain. The `test=True` parameter indicates this is a `test` setup. Then, we define the faucet used to fund the wallet with test tokens. We then define the domain for the uAgent's name registration and finally proceed to request funds from the faucet to ensure the agent's wallet has sufficient funds to operate. Remember that you need to provide the `name`, `seed`, `port`, `endpoint` and `DOMAIN` parameters to correctly run this code! 

5. We continue and define Bob's functions. We start with a `register_agent_name()` function which registers Bob's name within the blockchain using the uAgent's wallet, address, name and domain parameters:
    ...
    Here, we defined the `register_agent_name()` function. It registers the uAgent's name on the blockchain when the uAgent is initialized. The function uses the name service contract to register `bob-0.example.agent` uAgent using the `my_wallet` wallet. **It is important that you provide only lower case letters for your agent's `name` and `DOMAIN` parameters as otherwise you will face issues the moment you run your uAgent**.
...
Let's now define the code for our second uAgent, Alice.
...
3. We proceed and initialize the uAgent:
    ...
    We initialize an uAgent named `alice-0` with a specified `seed`. The uAgent listens on port `8000` and has an `endpoint` for submitting messages.

4. We then need to define the domain of the uAgent. The `DOMAIN` specifies the domain for the uAgent communication. This domain is used to construct the full address of Bob:
...
5. Finally, we define the functions and behaviours for Alice:
    ...
    This `alice_interval_handler()` function runs at regular intervals of 5 seconds to send messages. The handler constructs Bob's address using the domain and sends a message to Bob. It combines `bob-0` name with the `DOMAIN` to form `bob-0.example.agent`. The uAgent logs the action and sends a message with the content "Hello there bob." to Bob uAgent.

    Remember that you need to provide the `name`, `seed`, `port`, `endpoint` and `DOMAIN` parameters to correctly run this code! **Additionally, it is important that you provide only lower case letters for your uAgent's `name` and `DOMAIN` parameters as otherwise you will face issues the moment you run your uAgent**.

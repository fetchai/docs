...
Envelopes are encoded in JSON format and sent via `HTTP 1.1 POST` requests to specific endpoints of receiving uAgents or Functions. Currently, the protocol supports only one standardized endpoint:
...
**In essence, Envelopes in the `uagents` Framework act as secure wrappers for messages, providing essential information for their secure and reliable transmission between uAgents within the Framework itself.**

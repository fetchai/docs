...
Envelopes are encoded in JSON format and sent via `HTTP 1.1 POST` requests to specific endpoints of receiving Agents or Functions. Currently, the protocol supports only one standardized endpoint:

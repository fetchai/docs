...
5. We can now proceed and create our uAgents using the `Agent` class:
...
9. We can now create a `bureau` object from the `Bureau` class and then add both uAgents to it so for them to be run together.

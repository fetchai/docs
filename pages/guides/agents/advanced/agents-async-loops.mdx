...
The following script depicts how to attach an agent to an external event loop and allowing it to run simultaneously with other asynchronous tasks.

Create a Python script and paste the below code into it: `touch external_loop_attach.py`
    ...
    This script demonstrates how to run an agent using an external event loop. We first import the required libraries for this script to be run correctly. We then proceed and instantiate an agent called `looper` using a `seed`. Remember that you need to provide a seed within the `<YOUR_SEED>` field parameter. A Bureau is then created to manage our agents and we add the `looper` agent to the `bureau`.

    We define a `startup()` function decorated using the `.on_event("startup")` decorator. The function is triggered when the agent is started and it logs a message indicating the agent has begun operating. Similarly, the `shutdown()` function is triggered when the agent shuts down, logging an appropriate message.

    We go on and define a function `coro()` which simulates a separate, long-running task that will print `"Doing hard work..."` every second. This task runs independently of the agent and showcases the capability of handling multiple simultaneous tasks. We then attach both the agent and the external task (`coro`) to the same event loop using `loop.create_task()`. This allows both the agent and other tasks to execute simultaneously in an asynchronous way.
    ...
    The last line creates a context manager that suppresses `KeyboardInterrupt` exceptions, which are typically raised when the user presses `Ctrl+C` to stop the program. This ensures that the program can shut down without printing a traceback or throwing an error when the user stops it manually.

### Script 2

The goal of this script is to create an agent that runs tasks inside an external event loop. The agent can execute certain actions (like printing messages or responding to events) while simultaneously performing a separate background task.

Let's start by creating a Python script and paste the below code into it: `touch external_loop_run.py`

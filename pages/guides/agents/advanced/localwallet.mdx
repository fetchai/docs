...
To transact with FET on the Fetch.ai Ledger, you will need to manage your wallet with your uAgent. We use the Cosmpy library for ledger actions, coupled with in-library support for `cosmpy` in `uagents`.
...
## An uAgent that checks for a transaction, and sends funds

A simple uAgent that creates, or loads in a wallet, then validates a transaction has been received.
...
Validating a transaction with Cosmpy and uAgents is really easy, and to test the above uAgent you just need to replicate the `Models` and send a `RequestWithTX` in any function in another uAgent:

...
Let's try and keep this simple 🎉:
 ...
 - **Agents**: These are programs that work as you instruct them. Classically, they are part of a multi-agents system where they have a task and communicate with other agents. But their primary use in this case is to offer a functionality that can be accessed by DeltaV.
...
Once you get familiar with the uAgents Framework toolset for Agents development, you can then get started and start building your first [Agent Function ↗️](/guides/agents/intermediate/agent-functions). You create Agents offering some functionality which is then made discoverable on Agentverse to any other users, Agents and the AI Engine. For a better comprehension of Functions, how to register them on Agentverse, and how these play a role within the AI Engine, have a look at the following resources:
...
In a nutshell, AI Engine indexes Agents, and translates human input into **objectives**. It may sound like a search engine, but it does much more. Have a look at the following [resource ↗️](/concepts/ai-engine/powering-connections-and-smart-operations-in-deltav) to better understand how this process works.
...
Okay, take a quick break then let's create the second Agent; it's time to introduce multi-agents workflows ☕.

...
Let's try and keep this simple 🎉:
 ...
 - **DeltaV**: It is an assistive chat interface that allows human input to interact with Agents registered through the Agentverse.
    ...
    <Callout type="warning" emoji="⚠️">
       Agents are <i>programs</i>, whereas <i>functions</i> are specific Agents Functions your Agents offer.
    </Callout>
...
Zebra". The AI Engine would then look for an Agents that could give you a summary, as well as another Agents that could provide information about zebras. The AI Engine understands that the **Summary Agents** requires text to parse and summarise. So it would contact the **Zebra Agents** and get the information from it. The AI Engine would then call the **Summary Agents** with the **Zebra Agents** information so that the former would return the summarized text.
...
Of course, this could be more complicated. What if every sensor on a car was an Agents? You could ask your car for
diagnostics on a system, and each component in that system would independently give its status - making everything extremely complex. That's where the power of the AI Engine comes in. It understands the schema of each Agents, so there would be little need for each component to be compatible in communication with each other.
...
Now that the Agents are registered and discoverable - let's move on to the second part of the diagram. The second part of the diagram introduces the flow of the AI Engine. This shows the role of a user who has entered a search term into DeltaV.
    ...
    2. The AI Engine has an internal loop that updates Agents Functions registered and indexed.
    ...
    6. The AI Engine sees that Agents (A) needs some text to perform its functions.
    ...
    9. Data is passed to Agents (A) which returns a summary of the data.
...
If you've never installed anything for Agents development, checkout these guides:
...
### Create your first Agents: the text sentiment Agents
...
In this guide, our first Agents takes any text and returns a sentiment of contents with **openAI**.
...
<CodeGroup dynamic hasCopy isLocalHostedFile digest='4062beca9a7cafff6002e8e8ff9c7a28'>
...
We need a way for our Agent to receive messages. We do this by creating a function for our Agent to handle all
 ...
 our Agents receives a message matching the `SentimentResponse` message data `Model` we previously defined. Internally
...
<CodeGroup dynamic hasCopy isLocalHostedFile digest='669ae61db8d1e79422c0db593b959609'>
...
compliment one another.
    ...
    <Callout type="info" emoji="ℹ️">
       Check out the [Communicating with other Agent ↗️](/guides/agents/communicating-with-other-agents) guide to check how Agents communication works without the AI Engine.
    </Callout>

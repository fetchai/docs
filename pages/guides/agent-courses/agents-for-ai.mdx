...
# uAgents 101 for AI Engine

## Overview

Welcome to **uAgents 101 for AI Engine**! This course is designed to introduce you to the overall system of Fetch.ai, and shows how to
build uAgents to be accessible to the AI Engine and integrate with other projects. This course is a parallel track to
[uAgents 101 ↗️](/guides/agent-courses/introductory-course) which instead focuses only on uAgents creation.
...
At any stage, if you encounter issues or have questions about specific terms or topics, our support team is available on [Discord ↗️](https://discord.gg/fetchai) to assist you.
    ...
    <Callout type="info" emoji="ℹ️">
      Checkout the [AI Engine package ↗️](https://pypi.org/project/uagents-ai-engine/) to download it and start integrating your uAgents with this tool!
...
Fetch.ai is developing a platform to help build an AI-enabled decentralized digital economy. uAgents are
programs that can make choices on their own for individuals, companies, and devices. uAgents are the actors and the
heart of Fetch.ai Ecosystem.

To build your solution on Fetch.ai you need to understand four concepts: [uAgents ↗️](/concepts/agents/agents), [Agentverse ↗️](/concepts/agent-services/agentverse-intro), [AI Engine ↗️](/concepts/ai-engine/ai-engine-intro) and [DeltaV ↗️](/concepts/ai-engine/deltav).

### uAgents, Agentverse, AI Engine and DeltaV

Let's try and keep this simple 🎉:

 - **uAgents**: These are programs that work as you instruct them. Classically, they are part of a multi-agent system where they have a task and communicate with other uAgents. But their primary use in this case is to offer a functionality that can be accessed by DeltaV.

 - **Agentverse**: It is a managed hosting platform, an online IDE, and an entry point to AI Engine for uAgents. You would register your uAgents on Agentverse to be a Function in the network.

 - **AI Engine**: The AI Engine is a system that combines uAgents with human-readable text input to create a scalable AI infrastructure that supports Large Language Models (LLMs). It's an AI system that can convert human text into actionable (computer) commands.

 - **DeltaV**: It is an assistive chat interface that allows human input to interact with uAgents registered through the Agentverse.

    <Callout type="warning" emoji="⚠️">
       uAgents are <i>programs</i>, whereas <i>functions</i> are specific uAgent Functions your uAgents offer.
    </Callout>

In a nutshell, AI Engine indexes uAgents, and translates human input into **objectives**. It may sound like a search engine, but it does much more.
...
Zebra". The AI Engine would then look for an uAgents that could give you a summary, as well as another uAgents that could provide information about zebras. The AI Engine understands that the **Summary uAgents** requires text to parse and summarise. So it would contact the **Zebra uAgents** and get the information from it. The AI Engine would then call the **Summary uAgents** with the **Zebra uAgents** information so that the former would return the summarized text.
...
Of course, this could be more complicated. What if every sensor on a car was an uAgents? You could ask your car for
diagnostics on a system, and each component in that system would independently give its status - making everything extremely complex. That's where the power of the AI Engine comes in. It understands the schema of each uAgents, so there would be little need for each component to be compatible in communication with each other.
...
Here, **two separate** processes are happening. The first part of the sequence diagram registers two uAgents:

 1. Firstly, uAgents register to a smart contract thus effectively paying to be discoverable.
 2. uAgents then register as a Function on Agentverse (you'd usually do this manually).

Now that the uAgents are registered and discoverable - let's move on to the second part of the diagram. The second part of the diagram introduces the flow of the AI Engine. This shows the role of a user who has entered a search term into DeltaV.

    1. A query comes in and it is sent to the AI Engine.
    2. The AI Engine has an internal loop that updates uAgents Functions registered and indexed.
    ...
    5. It now requests addresses from [Almanac ↗️](/references/contracts/uagents-almanac/almanac-overview) to message these uAgents.
    6. The AI Engine sees that uAgents (A) needs some text to perform its functions.
    ...
    9. Data is passed to uAgents (A) which returns a summary of the data.
...
## Okay, let's build a couple of uAgents!
...
If you've never installed anything for uAgents development, checkout these guides:

    - [Setup the development environment ↗️](/guides/agents/installing-uagent#development-tools)
    - [Install uAgents locally ↗️](/guides/agents/installing-uagent#install-with-pip)

### Create your first uAgent: the text sentiment uAgent

We're going to run our uAgent locally, and set up a Mailbox on Agentverse.
...
In this guide, our first uAgent takes any text and returns a sentiment of contents with **openAI**.
...
Okay, we've got the whole code block above for our first uAgent. This uAgent does not run on Agentverse currently, as
...
We can now initialize the uAgent and define the [Protocol ↗️](/references/uagents/uagents-protocols/agent-protocols). To read more about **uAgent objects**, please check out [the reference docs ↗️](/references/uagents/uagents-api/agent#agent-objects).
...
We need a way for our uAgent to receive messages. We do this by creating a function for our uAgent to handle all
 incoming messages from other uAgents. We use `on_message()` decorator which activates the `sentiment()` function once
 our uAgent receives a message matching the `SentimentResponse` message data `Model` we previously defined. Internally
...
We can finally run the uAgent at the bottom of the script:
...
Including the protocol in the uAgent effectively allows other uAgents to know how this uAgent expects communication.
...
### Creating a Mailbox for Sentiment uAgent

A **mailbox** is a service provided by Agentverse that allows uAgents to receive messages when they're not running online.
`uagents` library will poll for new messages (and old unread ones) when your uAgent is running. Pretty handy.

Let's go to [Agentverse ↗️](https://agentverse.ai/agents/local) and create a Mailbox for this uAgent.

First of all, head over to the **My Agents** tab and click on the **Local Agents** tab.

Then, click on **Connect Local Agent** button and insert the **address** of the uAgent you want to retrieve.

![](../../../src/images/guides/uagent/servicefordungeons_1.png)

Let's paste in the uAgent's address we printed before to screen.

![](../../../src/images/guides/uagent/servicefordungeons_2.png)

Once the uAgent will be successfully fetched, add a **Name** to it.

A new **Mailbox API Key** will be provided for the local uAgent. Let's first copy the new Mailbox API Key and then update the `AGENT_MAILBOX_KEY = "AGENT_MAILBOX_KEY"` inline.

Now, you can restart your uAgent, and it will be registered to the Mailbox.
...
We're going to need to create an **uAgent Function** on Agentverse; to get a comprehensive overview of what uAgent Functions visit [our dedicated guide ↗️](/guides/services/services). To get the idea, an **uAgent Function** refers to an uAgent enrolled within the Agentverse, and made discoverable by other uAgents and users for interaction, which is retrievable through natural language queries. For instance, a Function could be an uAgent helping in booking an airplane ticket to some destination. Successfully registered uAgent Functions are discoverable and accessible through DeltaV and users can engage with them using natural language queries.

Let's go to the [Agentverse ↗️](https://agentverse.ai/) and head over to the **My Agents** tab and select the uAgent we have fetched above through **Connect Local Agent** button. A page will open showing your uAgent Profile details.

![](../../../src/images/guides/101/sentimentagent_agent_overview.png)

Click on **Deploy** tab to start creating an uAgent Function retrievable via DeltaV. Provide all the needed information to correctly register the Function. Remember that descriptions need to be as detailed as possible.

The following is the uAgent Function for the **SentimentAgent**:
...
Let's head over to DeltaV and communicate with our uAgent.
...
DeltaV tells us AI Engine has found an uAgent Function for our query, and checks we want to send the information we defined.

![](../../../src/images/guides/101/3_sentiment_deltav.png)

Then, DeltaV get the response from the uAgent Function and returns this to the user.

Your uAgent running locally should provide a similar output to this one below:
...
Okay, take a quick break then let's create the second uAgent; it's time to introduce multi-agent workflows ☕.

### Create the Webpage Summary uAgent

The next uAgent in the chain is the **Summary uAgent**. Let's name the script `web_summary_agent.py`. To do so run:

    <CodeGroup hasCopy isOSFile>

        ```py copy filename="mac"
        touch web_summary_agent.py
        ```

        ```py copy filename="windows"
        echo. > web_summary_agent.py
        ```
...
This uAgent has the same structure as the sentiment uAgent we previously defined.
...
This uAgent is simple again, hopefully you're seeing a pattern here. These uAgents while independent of each other can
compliment one another.

    <Callout type="info" emoji="ℹ️">
       Check out the [Communicating with other uAgents ↗️](/guides/agents/communicating-with-other-agents) guide to check how uAgents communication works without the AI Engine.
    </Callout>

Aside from the different logic, the primary difference in `web_summary_agent.py` is that this uAgent does not rely on a separate function, and wraps the whole logic in the message handler.

Let's run this one too. Again, follow the steps for the previous uAgent and go and register a Mailbox and update this uAgent with the new Mailbox. Each uAgent must have a unique [Mailbox ↗️](/guides/agents/intermediate/mailbox).

## One more uAgent Function 🎵

The following is the Function for the **Summary_agent**:

![](../../../src/images/guides/101/service_url_summary.png)

This one is quite simple, but we are defining this as a Secondary Function. A Secondary Function tells the AI Engine that this function should only be called to support another uAgent, and not as a primary response. You can learn more about this in our [guide ↗️](/guides/services/services#primary-and-secondary-functions)

We also need to go and update the Sentiment uAgent Function.

![](../../../src/images/guides/101/service_sentiment_a.png)

Oh my, that is quite the field description.

> "This uAgent will return a sentiment score from the provided text.
...
We invite you to dive deeper into the world of uAgents, Agentverse and AI Engine by exploring the dedicated documentation and [GitHub ↗️](https://github.com/fetchai/) repository. Join our [Discord ↗️](https://discord.com/invite/fetchai) and team up with other developers to participate in hackathons, collectively build projects, or simply have fun!

...
This course is designed to guide and instruct you through your first AI Agent development journey. As you go on through the course, you will be able to create increasingly complex agents and use cases. This guide is aimed at beginner programmers, but it contains a lot of information for experienced developers too. If you are unsure about certain terms or topics, contact us on [Discord ↗️] (https://discord.gg/fetchai) where a team member will be happy to help you.

## Introduction to AI agents

The [uAgents Framework ↗️](/concepts/agents/agents) is a lightweight framework enabling individuals and organizations to create and deploy AI Agents in a variety of contexts, including decentralized ones. **AI Agents** are programs designed to operate freely in a decentralized landscape based on the user's objectives. AI Agents can connect, search, and transact to create dynamic markets. These can be programmed with logic to interact with their environment and with other agents in the network and perform operations without requiring human participation.

Agents enable individuals and organizations to automate a wide range of workflows, from simple to complex, using a combination of artificial intelligence, API calls, blockchain technology and sophisticated business logic.

## Setup your machine 🛠️

For this course we will be using [Python ↗️](https://www.python.org/downloads/) programming language. Plenty of programming languages are available, but we think Python is particularly good to learn and build great AI Agents.
...
We recommend starting with Homebrew as it helps you to install all other software easily. Homebrew is a package manager for MacOS that simplifies the installation and management of software and packages on Mac computers via the command line. You can install it by simply running this command in your terminal; you can verify it [here ↗️](https://brew.sh/):

    ```
    /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
    ```

Then, let's ensure Homebrew is updated. Just run:
...
Now, you need to install PyEnv, which is a simple tool to manage multiple versions of Python. Just run:
...
Now, let's ensure the global version of Python you are working with is not the default one in the system. Run the following:
...
You now need to install Poetry. Poetry is used for managing Python project dependencies, handling virtual environments, packaging, and publishing Python libraries or applications. You can install Poetry by running the following command in your Zsh terminal:
...
You now have all our necessary tools installed and you are ready to initialize your project. First, let's create a working directory and initialize Poetry 🎉!
...
You can ensure you're in the correct directory by checking your current path. Just run:
...
The [uAgents Framework ↗️](https://pypi.org/project/uagents/) is the fundamental infrastructure in Fetch.ai's ecosystem that is essential for the creation and deployment of decentralized AI Agents. It serves as a base system that provides the tools, protocols and functionalities necessary for the development of autonomous agents. The framework is integrated with several key components of the Fetch.ai ecosystem, including the [AI Engine ↗️](/concepts/ai-engine/ai-engine-intro) and the [Almanac contract ↗️](/references/contracts/uagents-almanac/almanac-overview).

Users can provide natural language input via the [DeltaV ↗️](/concepts/ai-engine/deltav) chat interface to request the execution of a task. This input is translated by the AI Engine which works out the required goals and translates the input into a series of executable tasks. These tasks are then assigned to the most adequate AI Agents registered in the Almanac contract. This is a repository where important information about registered agents is stored to improve communication between agents within the Fetch.ai network. The Framework is also integrated with [Agentverse ↗️](/concepts/agent-services/agentverse-intro), a cloud-based platform that provides developers with a single environment for developing, managing and deploying AI Agents.
...
AI Agents developed with the uAgents Framework are identified by **two types of addresses**:
    ...
    1. **uAgent Address**: this is the primary agent identifier. It allows it to interact with other agents, exchange messages, and participate in decentralized network activities to ensure secure communication.
...
These addresses serve as identifiers for each agent in a decentralized environment. If you want to learn more, read our guide: [Getting an agent addresses 🤖📫 ↗️](/guides/agents/getting-uagent-address).
...
The [Almanac contract ↗️](/references/contracts/uagents-almanac/almanac-overview) is an important component in Fetch.ai's ecosystem. It operates as a blockchain-based repository where agents register, exchange information and establish communication. Registration in the Almanac is mandatory so that agents can participate in remote interactions and become discoverable through the Agentverse Explorer.
...
Let's create a new script for the agent communication example with `touch agent_communication.py`

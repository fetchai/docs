...
This course guides you through your first AI Agent development journey. As you move on, you will be able to create increasingly complex agents and use cases. This guide is aimed at beginner programmers, but it contains a lot of information for experienced developers too. If you are unsure about certain terms or topics, contact us on [Discord ↗️] (https://discord.gg/fetchai) where a team member will be happy to help you.

## Introduction to AI agents

The [uAgents Framework ↗️](/concepts/agents/agents) is a lightweight framework enabling individuals and organizations to create and deploy AI Agents in a variety of contexts, including decentralized ones. **AI Agents** are programs designed to operate freely in a decentralized landscape based on the user's objectives. AI Agents can connect, search, and transact to create dynamic markets. These can be programmed with logic to interact with their environment and with other agents in the network and perform operations without requiring human participation. Agents enable individuals and organizations to automate a wide range of workflows, from simple to complex, using a combination of artificial intelligence, API calls, blockchain technology and sophisticated business logic.

## Setup your machine 🛠️

For this course we will use [Python ↗️](https://www.python.org/downloads/) programming language. Plenty of programming languages are available, but we think Python is particularly good to learn and build great AI Agents.
...
We recommend starting with Homebrew as it helps you to install all other software easily. Homebrew is a package manager for MacOS that simplifies the installation and management of software and packages on Mac computers via the command line.

You can install it by simply running this command in your terminal; you can verify it [here ↗️](https://brew.sh/):
...
Now, you need to install PyEnv. It is a simple tool to manage multiple versions of Python. Just run:

    ```
    brew install pyenv
    ```

Once you have installed PyEnv you can configure its shell environment:
...
These commands configure your shell environment (specifically the **Zsh shell**) to work with PyEnv. These commands set up environment variables, modify the PATH, and initialize PyEnv so that you can easily manage and switch between different Python versions.

    <Callout type="info" emoji="ℹ️">
      You can verify all steps [here ↗️](https://github.com/pyenv/pyenv#installation).
    </Callout>

You are now ready to **install Python**. You need to install a version of Python 3.8 or above (for this example, we use version 3.10):
...
You can install Poetry by running the following command in your Zsh terminal:
    ...
    <Callout type="info" emoji="ℹ️">
      If you would like to learn more about Poetry, visit the [website ↗️](https://python-poetry.org/docs/#installation) where you can quickly find answers to all your questions.
    </Callout>

### Initialize your project with Poetry

You now have all our necessary tools installed and you are ready to initialize your project. Let's create a working directory and initialize Poetry 🎉!
...
If you are happy with the install location for the project, go ahead and initialize Poetry:
    ...
    <Callout type="info" emoji="ℹ️">
      Follow the setup wizard to provide details including project name, version, author, license, and select dependencies (in your case, `uagents`).
    </Callout>
...
**With this, you have finalized the installation process. You can now start creating your first AI Agent!**

## Overview of the uAgents Framework

The [uAgents Framework ↗️](https://pypi.org/project/uagents/) is the fundamental infrastructure in Fetch.ai's ecosystem for the creation and deployment of AI Agents. It serves as a base framework providing the tools, protocols and functionalities necessary for the development of autonomous agents. The Framework is integrated with several key components of the Fetch.ai ecosystem, including the [AI Engine ↗️](/concepts/ai-engine/ai-engine-intro) and the [Almanac contract ↗️](/references/contracts/uagents-almanac/almanac-overview). This is a repository where important information about registered agents is stored to improve communication between agents within the Fetch.ai network. The Framework is also integrated with the [Agentverse ↗️](/concepts/agent-services/agentverse-intro), a cloud-based platform that provides developers with a single environment for developing, managing and deploying AI Agents.

### Key concepts

#### Addresses

AI Agents developed with the uAgents Framework are identified by **two types of addresses**:
    ...
    2. **Fetch Address**: this cryptographic public address is linked to the agent and its wallet on the Fetch.ai blockchain; it enables various functionalities, including interacting with the Fetch ledger, registering in the Almanac contract and performing operations such as token or asset transfers on the blockchain.

These addresses serve as identifiers for each agent within the Fetch.ai Ecosystem.

    <Callout type="info" emoji="ℹ️">
      Check out the [Getting an agent addresses 🤖📫 ↗️](/guides/agents/getting-uagent-address) guide to learn more on this topic.
    </Callout>

#### Storage

AI Agents can store information in a **JSON file** that they can freely retrieve when needed. **Storage** is critical for agents to maintain state, remember past interactions and make informed decisions based on historical data.

    <Callout type="info" emoji="ℹ️">
      If you wish to learn more, explore our [Using agents storage function ↗️](/guides/agents/storage-function) guide or the [agents storage ↗️](/references/uagents/uagents-protocols/storage) documentation. If you are new to JSON please see an example [here ↗️](https://www.w3schools.com/whatis/whatis_json.asp).
    </Callout>

#### Protocols

The uAgents Framework provides comprehensive support for the organization of message types and their handlers within **protocols**. Protocols are sets of rules that govern the transmission, reception and interpretation of data between devices or systems. They define the communication format, timing, sequence and error handling. Protocols enable standardized communication and ensure accurate and reliable data exchange. Agents using the same protocol can communicate directly with each other, which promotes efficient interaction within the system.

If you want to become an expert in Fetch.ai's agent technology, we strongly recommend that you read our [protocols ↗️](/references/uagents/uagents-protocols/agent-protocols) documentation.

#### Exchange protocol

The **Exchange Protocol** facilitates efficient communication between agents by using standardized messaging techniques. Messages are packaged in envelopes, encoded and transmitted to specific endpoints via HTTP. Messages consist of key-value pairs in JSON format and are packaged in envelopes with metadata. **Envelopes** contain:
...
The agents registered in the Almanac provide service endpoints for remote communication, which contain weighted parameters that enable effective interaction. As a central hub, the Almanac facilitates the discovery of endpoints based on these assigned weights. This structured approach promotes efficient agent interactions and a robust environment for the development of AI agents within Fetch.ai's decentralized network. Users can navigate the Agentverse Explorer and filter agents based on different protocols customized to their specific application requirements.
...
Let's get Alice to do something! We're going to get Alice, on startup, to introduce itself and provide its address by printing both, on the terminal window. We can add a `on_event("startup")` decorator to make the agent run a function when run.
...
The output of the above would look similar to:
    ...
    <Callout type="info" emoji="ℹ️">
      Checkout our [Getting an agent addresses 🤖📫↗️](/guides/agents/getting-uagent-address) guide for an indepth understanding of these topics.
    </Callout>
...
We can now introduce how to enable effective communication between different agents. To do so, we need to introduce the `Model` class which allows to create a structured message format for messages to be exchanged between the agents.
...
We first import, then initialize our agents and include a function ensuring that agents have non-zero balances in their wallets. This function will check if you have enough tokens to register in the Almanac. If not it will add tokens to your Fetch wallet address. AI Agents can communicate by querying the Almanac contract and retrieving an HTTP [endpoint ↗️](/references/contracts/uagents-almanac/endpoints) from the recipient agent. Therefore, we need to specify the service endpoints when defining an agent at registration.
...
Just like we did before, let's create a Python script for this example: `touch almanac_registration.py`.
...
AI Agents can also interact **remotely**. To achieve a remote communication, we simply need an agent's address and query the rest of its information in the Almanac contract. You can create two agents operating on separate ports and terminals within the same device. This mirrors a real-world scenario in which these agents communicate across different geographic locations.

In this example, we provide two scripts for two agents. To establish a line of remote communication both agents need to be registered on the Almanac contract and need to have non-zero balances in their Fetch wallet addresses.
...
We now want to show how to set up the code to create a **restaurant booking service with two **AI Agents**: a `restaurant` with tables available and a `user` requesting a table availability.

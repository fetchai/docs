...
Let's get started! For this course we will be using the Python programming language [Python ↗️](https://www.python.org/downloads/). There are plenty of programming languages available, but we think Python is particularly good to learn with, but also build great to build AI agents with too.
...
Now that we've got all our necessary tools installed, let's create your working directory and initialize Poetry 🎉
...
If you're happy with the install location for the project, let's go ahead an initialize Poetry:
...
Similarly, we also need to define a script for `bob` so to create a remote communication with `alice` agent. Once again let's use the `touch remote_bob.py` command:

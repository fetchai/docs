...
We now introduce the `Model` class, enabling effective communication between different agents. The `Model` class allows us to establish a structured message format, which provides an adequate framework for passing messages:
...
We then need a way for Bob to receive these messages, wecan do this but creating a function for `bob` to handle all incoming messages from other agents. We will do this through a `on_message()` decorator that will activate the `message_handler()` once `bob` receives a message of type `Message`:

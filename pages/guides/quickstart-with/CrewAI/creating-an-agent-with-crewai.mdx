...
Fetch.ai provides a dynamic communication layer that allows you to modularize components into distinct [uAgents ↗️](/guides/agents/getting-started/whats-an-agent)️. In this ecosystem, uAgents act as microservices, programmed to communicate either with other uAgents or with humans. While other frameworks also create Agents, here we'll explore how Fetch.ai’s unique uAgents architecture can complement and extend CrewAI's functionality.

By using Fetch.ai’s uAgents to represent various elements of your Crew, your project can be designed to interact with [third parties ↗️](/guides/agents/intermediate/communicating-with-other-agents)️ to enhance both flexibility and scalability and increase potential economic benefits.

In this guide, we'll walk you through a simple CrewAI example to understand its fundamentals and then demonstrate how we can enhance this model by integrating Fetch.ai uAgents for advanced communication and workflow management.

## A simple CrewAI example

Let's use a shortened example that CrewAI provides; in this example we define an uAgent and a Task. At a high level here, uAgent is defining the LLMs "personality" and the Task is what that LLM is to solve.
    ...
    # Define your uAgents with roles and goals
    researcher = Agent(
...
This is a nice feature; we love the idea of having many tasks and then orchestrating to uAgents which can execute them.
But what if we want to create a workflow, perhaps one where information is passed back and forwards?

Next, let us show you two uAgents, which communicate, just like that.

## A simple communication with uAgents

Fetch.ai has the concept of an uAgent where this is an uAgent that is the component that links uAgents together. 

You can read more about uAgents communication in our [guides ↗️](/guides/agents/intermediate/communicating-with-other-agents)
...
Our first uAgent is simple; it sends a message every two seconds to a static address. When this uAgent receives a message, it prints that to log:
...
This first uAgent introduces a few core concepts you will need to be aware of when creating any uAgent.

uAgents are defined with the `Agent` class:
...
A `seed` is a unique phrase which `uagents` library uses to create a unique private key pair for your uAgent. If you change your `seed` you may lose access to previous messages, and also, the uAgent's address registered to the Almanac will change subsequently. The `port` allows us to define a local port for messages to be received. The `endpoint` defines the path to the in-built Rest API. The `name` defines the name of the uAgent.
...
The `Model` defines the object sent from uAgent to uAgent and represents the type of messages the uAgent is able to handle. For explicit communication, both uAgents must handle the same `Model` class. `Model` is the base class that inherits from Pydantic BaseModel.

With the `fund_agent_if_low(agent.wallet.address())` function, uAgents will ultimately pay for discoverability as the economy of uAgents matures. There is a placeholder for registration here.

Finally, uAgents have two decorated functions. 
...
The second one is `agent.on_message()` which is a little different; when the uAgent receives a message at the `endpoint` we defined earlier, the `uagent` library unpacks the message and triggers any function which handles that message; in our case, the `agent.on_message()` function:
...
uAgent two doesn't do anything different to uAgent one; it has different args for the uAgent instantiation, and instead of sending a message `on_event("startup")`, agent two just logs its address to screen. Whenever uAgent two receives a message matching `Message` data model, it will send a response to the sender.
...
Okay, let's now run these uAgents.

### Running the uAgents

Let's run the second uAgent's script first using this command: `poetry run python agent2.py`

**We must run the second uAgent first to get its unique address**. This is shown in output in the log. Let's update `agent1.py` script by filling the `RECIPIENT_ADDRESS` field with the address of the second uAgent from of the output we previously got by running `agent2.py` script.
...
Then, let's run the script for the first uAgent using this command: `poetry run python agent1.py`
...
Let's go further now and change our uAgents scripts by splitting the logic of the CrewAI example above. Let's have one uAgent that creates a task and another uAgent which fulfills it.  

### uAgent one: Senior Research Analyst uAgent
...
- `run_process(city: str)`: This method coordinates the execution of the created task. It initializes a Crew process that includes the research uAgent and the task, allowing it to gather results efficiently.
...
### uAgent two: Research Asking uAgent

The `research_asking_agent` uAgent is designed to initiate requests for city-specific research information and manage responses related to research reports.

- Models: `CityRequestModel` for storing the city name and `ResearchReportModel` for the research findings.

- Startup Event: The `on_startup` function logs the uAgent's name and address, then sends a message to a target uAgent with the default city (London).
...
This uAgent operates continuously, enabling effective communication and retrieval of city-related research insights.
...
You should get something similar to the following for each uAgent:

...
```python copy filename="second_agent.py"
from uagents.setup import fund_agent_if_low
from uagents import Agent, Context, Model
...
```python copy filename="agent_one.py"
from uagents import Agent, Context, Model
from uagents.setup import fund_agent_if_low
...
```python copy filename="langchain_agent_one.py"
...
```python copy filename="langchain_agent_two.py"
from langchain_community.document_loaders import PyPDFLoader
import os
from langchain_community.vectorstores import FAISS
from langchain_openai import OpenAIEmbeddings
...
Run `poetry run python langchain_agent_two.py` first and then `poetry run python langchain_agent_1.py`.

...
## something simple with langchain
...
## something simple with Fetch.ai

Fetch.ai has the concept of an agent which at a base level an agent cannot do what langchain does, but an agent is the
componenet that links them together.
...
In the next part of this introduction, we will create a multi agent pipeline where we split the logic of the PDF agent into three more agents, one which verifes a PDF, the nexrt loads and splits a PDF and the final agent uses faiss to do the similarity search. 
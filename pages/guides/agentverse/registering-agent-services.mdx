...
On the [Agentverse ↗️](https://agentverse.ai/) within the **My Agents** tab we previously defined our agent. In here, we can now define the Agent Service we want to provide to users. In this context, **Services are developed as _Agents Functions_** directly within the Agent Editor window appearing when developing your Hosted Agent. This because agents provide specific functionalities to users depending on objective requested.

...
`_strptime` is a function within the Python `datetime` module used for parsing strings into datetime objects. It's utilised internally by the `strptime()` method of the `datetime` class.
        ...
        **Example**:
          ...
          # Format string specifying the format of the input date string
          format_string = "%Y-%m-%d"

          # Call _strptime function directly to parse the date string
          parsed_time = _strptime._strptime_time(date_string, format_string)

          # Create a datetime object from the parsed time tuple
          date_object = datetime(*parsed_time[:6])

          print(date_object)  # Output: 2024-04-23 00:00:00
          ```

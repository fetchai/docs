...
**The modules available with an explicit import statement include**:

- `uagents`: build fast and lightweight agents for decentralized scenarios using the μAgents framework. Checkout the [uagents package ↗️](https://pypi.org/project/uagents/). **Available classes**: `Model`, `Context`, `Protocol`.

- `json`: simply interact with JSON data structures. **Available functions**: `dumps`, `loads`.

- `requests`: interact with HTTP requests and responses. **Available functions**: `get`, `post`, `put`, `patch`, `delete`.

- `cosmpy`: a Python library for interacting with Cosmos-based blockchains. Checkout the [CosmPy package ↗️](https://pypi.org/project/cosmpy/). **Full access to all functions and features**.

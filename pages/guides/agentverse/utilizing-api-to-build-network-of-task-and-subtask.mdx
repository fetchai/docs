...
TThis agents network helps users to read news of different types including **Categorical**, **Country** based or **Keyword** related news. The system operates based on a multiple layers structure of tasks and subtasks, each one of them playing a vital role in delivering personalized news content to users.
...
![](../../../src/images/guides/services/utilizing-api-to-build-network-of-task-and-subtask/news-reading-system-flowchart.drawio.png)

First of all, we have the **News Reading System**; this one acts as the gateway to the world of news. This layer enables the straightforward connection between the user and the latest headlines. When the user expresses interest in reading the news, the [AI Engine ↗️](/concepts/ai-engine/ai-engine-intro) drives the News Reading System which comes up into action, and initiates the process of fetching news articles for the user. Once the system understands your intent is to read news headlines, it will call upon the **Generate News Subtask**. At this point, users will be required to provide the news type they want to read via [DeltaV ↗️](/concepts/ai-engine/deltav).
...
With  the user's news type preference in hand, the system then directs request to one of the specialized subtasks: the **Generate news_type Subtasks**. These subtasks are like expert news gatherers, each dedicated to a specific type of news. Whether user is keen on categorical news, regional updates, or keyword-specific articles, there’s a subtask ready to fetch the latest headlines tailored to your interests.

All **tasks** and **subtasks** redirections are handled by **AI Engine**, which diligently processes user preferences and triggers the appropriate subtask to fulfil the user's requests. By providing clear and concise field descriptions for each task and subtask, the AI Engine will ensure straightforward navigation through the News Reading System and operations, making it easy for users to stay informed on the topics that matter the most to them. With this system, the process of getting the news user wants has never been easier!

Sit back, relax, and let DeltaV and the AI Engine bring the top latest headlines to you!
...
The respective **Generate news_type subtask** will ask for _categorical_, _country_ or _keyword_ based news type, and will generate top 10 news related to news subtype provided.

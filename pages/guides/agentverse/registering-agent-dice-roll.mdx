...
In the [previous section ↗️](/guides/agentverse/registering-agent-coin-toss) you have got familiar with the creation of a coin toss agent function that can be registered as a Function and then used via [DeltaV ↗️](/concepts/ai-engine/deltav).

In this example we are going to try out another use case: a **dice roll agent**.

    <Callout type="info" emoji="ℹ️">
      Checkout the [Agentverse Functions ↗️](/guides/services/services) guide for additional information needed for Agents Functions registration on the Agentverse and Fetch network.
    </Callout>
...
Similar to the [Agentverse Functions: register a coin toss agent as a Function 🪙 ↗️](/guides/agentverse/registering-agent-coin-toss#register-your-coin-toss-agent), let's navigate to the [Agentverse: My Agents ↗️](https://agentverse.ai/) tab. Here, click on your agent to show the Agent Editor and then click on the **Deploy** tab to start registering your agent function. Fill the form out as follows:

![](../../../src/images/guides/agentverse/registering-agent-dice-roll/new-service-dialog.png)

    - **Function title**: just the name of your Agent Function. In this example let's call it _Dice roll function_.
    ...
    - **Application**, **Protocol**, **Model** and **Field descriptions** will be automatically populated based on the source code of [your dice roll agent ↗️](/guides/agentverse/registering-agent-dice-roll#run-your-dice-roll-agent)
...
After navigating to the chat interface, you will be asked to select an option. As your objective task (`Roll a dice`) specified on the previous screen contained words related to the [description of your dice roll agent ↗️](/guides/agentverse/registering-agent-dice-roll#register-your-dice-roll-agent), your **DiceRoll** Function is listed as an option. Let's select it.
...
After your function has been executed you can see your dice roll results.

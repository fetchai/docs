...
The Hugging Face Inference API allows access to thousands of models for specific objective execution. In this tutorial, we will integrate the Hugging Face API with the Agentverse Agent, enabling users to select their preferred **text-classification** model. Users can choose from the top downloaded models and make queries to them. This system consists of multiple layers of Primary and Secondary functions, with the Hugging Face system as the Primary Function and Hugging Face request and model list as Secondary Functions. For a better understanding of the workflow, please refer to the documentation below.
...
This Agent helps users with their queries for a text-classification model of their choosing. The agent always prioritizes Hugging Face request secondary function. Below is the script for creating our agent on [Agentverse ↗️](https://agentverse.ai/agents) using the **+ New Agent** button.
        ...
        class HF(Model):
            response : str # This class has a single attribute 'response' that holds the string response from the secondary subtask.
...
This agent helps hugging face system to handle hugging face request to user and calculates the response. The agent has two fields i.e. `model_id` (to which query has to be made) and `query` (Question needed to be asked to the model). For `model_id`, this always prioritizes `Model List` secondary function to get list of available model of a specific type.
...
## Setting up Agent Functions

Go to the [Agentverse ↗️](https://agentverse.ai/) and create new Agent Functions for all three agents created [above ↗️](/guides/agentverse/huggingface-text-classification-model-agent#setting-up-agents). You can do so within the **My Agents** tab, by clicking on desired agent and accessing its information. There, you will need to click on the **Deploy** tab and provide all needed details required for successful Agent Function registration. For additional information on Agent Functions check the following resources: [Agentverse Functions ↗️](/guides/services/services) and [Register an Agent Function on the Agentverse ↗️](/guides/agentverse/registering-agent-services)

The properties of Functions for each agent are listed below.

### Hugging Face System

![](../../../src/images/guides/services/service-guide/hugging_face_1.png)

    - **Function title**: just the name of your Agent Function - in this example let's call it **Hugging Face System**.
    - **Description**: super important to be as detailed as you can, as reasoning AI Engine looks at descriptions to understand what your Agent Function does - in this example we can specify something like this: **This function helps user to give any query to the selected model and gets the response to their query. Always go for hugging face request secondary function for this primary function.**
                        **Secondary functions chronology**
    ...
    - **Field descriptions**: This field is super important to be detailed and is responsible for triggering secondary functions. In this example we can specify something like: _**Describes the response to the user query. Always go for Hugging face request subtask to get this field. Never ask this from user**_.

### Hugging Face Request

![](../../../src/images/guides/services/service-guide/hugging_face_2.png)

    - **Function title**: just the name of your Agent Function - in this example let's call it **Hugging Face Request**.
    ...
    - **Field descriptions**: this field is super important to be detailed and is responsible for triggering secondary function. In this example we can specify something like:
...
![](../../../src/images/guides/services/service-guide/hugging_face_3.png)
    ...
    - **Function title**: just the name of your Agent Function - in this example let's call it **Model List**.

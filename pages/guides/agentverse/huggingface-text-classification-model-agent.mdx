...
Create news [access token ↗️](https://huggingface.co/settings/tokens) in settings for reading access type. This API key will be used in agent scripts 
to fetch model lists and make query to respective models.
...
This Agent helps users with their queries for a text-classification model of their choosing. The agent always prioritizes Hugging Face request subtasks. Below is the script for creating our agent 
on [Agentverse ↗️](https://agentverse.ai/agents) using the New Agent button.
...
This agent helps hugging face system to handle hugging face request to user and calculates the response. The agent has two fields i.e. model_id (to which query has to be made) 
and query (Question needed to be asked to the model). For model_id this task always prioritizes Model List sub-task to get list of available model of 
a specific type.
    ...
    ```python copy filename="agent.py"
        ...
        async def handle_query(model_id, query):
            ...
            API_TOKEN = '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' # hugging face API token
...
This agent helps user to look for specific model with search keyword. The agent queries hugging face url to get top 5 downloaded model related to search
keyword. The agent returns list of models and user can select one they need.
...
Go to [AgentVerse Services ↗️](https://agentverse.ai/services) and create new services for all three agents created in section.(/guides/agentverse/huggingface-text-classification-model-agent#setting-up-agents).
...
![](../../../src/images/guides/agentverse/Hugging-Face-System/Hugging-Face-System-2.png)
    ...
    - **Field descriptions** : This field is super important to be detailed and is responsible for triggering subtask. - in this example we can specify something like 
                                **describes the response to the user query. Always go for Hugging face request subtask to get this field. Never ask this from user.**
...
![](../../../src/images/guides/agentverse/Hugging-Face-System/Hugging-Face-Request-2.png)
    ...
    - **Field descriptions** : This field is super important to be detailed and is responsible for triggering subtask. - in this example we can specify something like 
                                **_Model_id :_ Always go for model list subtask. Never ask this field to user.**
                                **_query :_ Describes the query user wants to ask to the model Always ask this to user after model_id is given by model list subtask ** 
...
![](../../../src/images/guides/agentverse/Hugging-Face-System/Model-List-2.png)
    ...
    - **Field descriptions** : This field is super important to be detailed and is responsible for triggering subtask. - in this example we can specify something like 
                                ** This is the search keyword of model user wants to ask answer to. Always ask this to user. This always gives list of options to the user. make user select one from these options. ** 
...
In this case we will ask for sentiment analysis model. Model_list will give us options from top 5 downloaded models and we need to select one option.
Hugging face request will ask us for the query and we want to ask to model and send the response to hugging face system.

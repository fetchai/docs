...
The Artificial Superintelligence Alliance (ASI) network facilitates CUDOS token reconciliation following the CUDOS network's merger into Fetch.ai’s ASI Alliance. This process allows users to migrate their CUDOS tokens to FET tokens based on a pre-agreed conversion rate via the [Companion App ↗️](https://companion.fetch.ai/fetchhub-4/reconciliation) UI. 
...
If CUDOS tokens were previously sent via IBC (Inter-Blockchain Communication) back to their native CUDOS Mainnet chain **after** it was decommissioned, then these tokens became locked forever. Users claim these locked tokens as FET ones via the [Companion App ↗️](https://companion.fetch.ai/fetchhub-4/reconciliation).

The most crucial aspect of claiming locked tokens is selecting the correct wallet account (with the correct ASI address) in the [ASI Alliance Web Wallet ↗️](/guides/fetch-network/asi-wallet/web-wallet/get-started) extension, otherwise users won't be able to see their claimable tokens in the Companion app UI and consequently claim them. The correct ASI account must be derived from the **very same** private key as the original sender's account used to transfer CUDOS tokens from the source chain (e.g. Osmosis) to the already decommissioned CUDOS Mainnet chain, which caused the tokens to become locked.

_**Users will need to import the correct ASI account into the ASI Wallet Extension**_! There are two possible scenarios here, depending on how the original sender's account was created (and respectively, how the user is accessing it):

- **Software wallet**: users will need to add the correct ASI account into their ASI Alliance Web Wallet extension by using the very same mnemonic seed phrase and HD Derivation path matching the original sender's address.
...
If all parameters are correctly provided, the user will end up with the correct ASI wallet account derived from the same private key as the original sender's account the user used when CUDOS tokens became locked.
...
7. Review that the amount displayed is correct, then proceed to submit the claim request by clicking the **Execute** button.
...
If users hold liquid CUDOS tokens on IBC-supported networks, they can convert them to FET tokens.

First, they will need to transfer them via IBC to the ASI Alliance Network, and then convert them to FET tokens using the Companion App UI. 

    <Callout type="info" emoji="ℹ️">
      At the moment, only CUDOS tokens on the **Osmosis** and **Archway** networks are supported. In addition, _CUDOS tokens on the Archway network will need to be first transferred over IBC to the Osmosis network, and then from there to the ASI Main network_.
   </Callout>
...
5. The available balance of CUDOS tokens should be displayed automatically based on the amount available of liquid CUDOS tokens on your connected wallet. 
6. Select the amount you wish to convert. 

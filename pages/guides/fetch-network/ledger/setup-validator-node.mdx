...
You should verify that you have some tokens available beforehand before trying to create a validator. The easiest way to do this is via the [CLI ↗️](/guides/fetch-network/ledger/cli/tokens).

Below, you can find an example of a typical CLI command that registers the node as running the validator:
...
This can be simple things such as the associated website for a validator or more consequential actions like altering the commission rate. In either case, should a validator choose to make this update they would send an `edit-validator` transaction to the network. These transactions can be created in a similar way to the `create-validator` transaction depicted above:
...
Whenever and if a validator wants to stop being a validator for any reason, they can unbond some or all of their staked tokens. **Unbonding refers to the process of withdrawing or removing a certain amount of staked tokens from a validator or a network after a defined waiting period**. This operation is carried out with the following CLI command:

...
testing. This document outlines the steps required in order to configure a `fetchd` network of 1 node (i.e. **single node network**).

## Network setup

<Callout type="info" emoji="ℹ️">
  These steps only need to be carried out once to setup the local network.
</Callout>
...
5. **Add the validator to the network**. Let's now set the initial state for the network, using the following command allocating `100000000000000000000` `stake` tokens to the validator which can be bonded:
...
6. **Generate a transaction**. Now, let's **generate a validator transaction**. To get your validator to sign the genesis block, and to agree that this is the correct genesis starting point, use the following command:

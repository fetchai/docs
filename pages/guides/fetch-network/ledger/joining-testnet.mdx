...
If you want to join a test network, you will need to have the **correct version of the `fetchd` ledger available on your system**.

## Using a local version

If you have correctly followed the [installation ↗️](/guides/fetch-network/ledger/installation) guide, then you should now have `fetchd` successfully installed in your path.
...
Check the [network page ↗️](/references/fetch-network/ledger/active-networks) for the list of supported versions for each network.
...
You can initialize `fetchd` by running the following command:
...
Finally, you will need to **connect `fetchd`** by getting it to connect to a seed node for the given network:
...
1. You would need to initialize a new Fetch.ai node named `my-first-fetch-node` with the chain ID `dorado-1` using the following command:
...
3. Then, you would start the Fetch.ai node with specific seed nodes on the Dorado testnet using the following command:

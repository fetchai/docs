...
The Command Line Interface (CLI) client provides all the capabilities for interacting with the [Fetch Ledger ↗️](/concepts#fetch-ledger-and-indexer), such as creating addresses, sending transactions and the governance capabilities. Before starting with the Command Line client, you need to follow the [installation instructions ↗️](/guides/fetch-network/ledger/installation) for the Fetch Ledger.

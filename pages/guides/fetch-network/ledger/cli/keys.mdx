...
To **create a new local key**, you would need to run the following command:
...
To use the hardware wallet address with the CLI, the user must first add it via `fetchd`. This process only records the public information about the key.

You can **import the key** by first plugging in the device and enter the device pin. Once you have unlocked the device, navigate to the Cosmos app on the device and open it.

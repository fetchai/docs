...
<Callout type="warning" emoji="⚠️️">
  The state sync feature is only available from `fetchd  v0.10.6` and later. Prior versions needed to either sync from scratch or restore a [chain snapshot ↗️](/guides/fetch-network/ledger/snapshots), which both could take hours before having the node fully synced.
</Callout>
...
Open this file in an editor and lookup for the `statesync` section.
...
To provide new nodes snapshots they can start from, existing nodes need to be configured to create these snapshots.

...
If you want to instruct the node to sync itself using a state-sync snapshot, it need some configuration in the `~/.fetchd/config/config.toml` file. To configure the new node, open this file in an editor and lookup for the `statesync` section.

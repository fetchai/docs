...
Once we have successfully compiled our contracts, we need to make sure to fill out the necessary instantiation message information under the `init` field in the `jenesis.toml` file.
...
To deploy all the contracts inside a profile, we have two options:
...
We can point to other contract addresses in any contract's instantiation message if required.

For instance, if we have contracts `A`, `B`, and `C` within our project, but contract `A` requires contract's `B` deployment address in its instantiation message and contract `B` requires contract's `C` deployment address, these will need to be deployed in the following order: `C`, `B`, `A`. In order to provide this information to `Jenesis` we will need to specify where exactly these contract addresses need to be inserted inside the instantiation messages. We can do this by writing the `$` symbol followed by the contract name in the corresponding field in the init parameters:

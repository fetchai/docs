...
**We will be exploring some of these commands below so to get you started!**

## Create a new project

You can create a new project using the `new` command in the following way:

    ```
    jenesis new my_project [--profile my_profile] [--network network_name]
    ```

This creates a new directory called `my_project`. You can use `--profile` and `--network` optional arguments to provide additional arguments for profile and network desired for your project. Whenever these arguments are not used, these will be set to `testing` and `fetchai-testnet` respectively.

By running the above command, a `jenesis.toml` file will be created within the directory for your project, containing the following information:
...
The `init` command is similar to the `new` command, but in this case, you will not need a project `name` argument since **this command is intended to run inside an existing project directory**:
    ...
    If you are using a cargo workspace, you just need to navigate to the top level of your project and run the `init` command shown above. This will create the `jenesis.toml` configuration file inside your workspace including all the relevant information from existing contracts.
...
At any time, **you can start or stop a local node by running** in the followong way:

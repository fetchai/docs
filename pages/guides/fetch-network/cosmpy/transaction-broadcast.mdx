...
CosmPy simplifies broadcasting transactions. After [creating a wallet ↗️](/guides/fetch-network/cosmpy/creating-wallet) you can send transactions by following the provided example. In it we first connect to the desired network then provide the desired destination address. The transaction is then broadcast using the `Ledger_client.send_tokens` function:
...
Once you have executed the script you will notice the console output is consistent with the code. We first see a print of the starting balance of both addresses, we then see the transaction hash along with the new balances for boch Alice and Bob:

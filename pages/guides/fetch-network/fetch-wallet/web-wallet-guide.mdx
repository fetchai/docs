...
Install **Fetch Wallet** (*version 0.16.0*). The Fetch Wallet is supported by all Chromium-based web browsers, including **Chrome**, **Brave**, **Edge**, and **Decentr**. You can install in your brave and chrome-based web browser by using following link:
...
![](src/images/guides/wallet/web-wallet_11.png)
    ...
    3. After that, You will then be taken to the main import screen, where you can name the key, specify the ETH address you are migrating from, which in this case is your Metamask ETH address (this is used to ensure that the private key matches), and finally copy the private key from your Metamask account.
...
**Organisation** - User can select the organisation from notification option.
...
The fetch wallet us new feature is the "".FET Domains", which allows you to add a domain with the .FET.

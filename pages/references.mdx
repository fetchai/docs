# References

<div className="nx-text-fetch-content">
  The place to get yourself around the more technical side of things used as a
  reference in all other sections of the documentation.
</div>

import { Grid2, Grid3 } from "../components/mdx";
import { GuideBox } from "../components/feature-guide-tabs";

## Contracts

<div className="nx-text-fetch-content">
  An overview of the contracts used during development.
</div>

<Grid2>
    <GuideBox
        content={{
            title: "Almanac",
            description: "The Almanac is a smart contract developed and deployed on the Fetch.ai blockchain which provides users with a direct way to query a particular uAgent's information.",
            path: "/references/contracts/uagents-almanac/almanac-overview"
        }}
    />

    <GuideBox
        content={{
            title: "Endpoints",
            description: "Endpoints are essential for remote communication between agents.",
            path: "/references/contracts/uagents-almanac/endpoints"
        }}
    />

</Grid2>

## uAgents Framework

<div className="nx-text-fetch-content">
  An overview of the constituting elements needed to carry out effectively tasks
  and communications between agents.
</div>

<Grid2>
    <GuideBox
        content={{
            title: "uAgents Protocols",
            description: "Protocols are used to facilitate communication and interaction between uAgents in the uAgents framework.",
            path: "/references/uagents/uagents-protocols/agent-protocols"
        }}
    />

    <GuideBox
        content={{
            title: "Exchange Protocol",
            description: "The Exchange Protocol defines a standardized method for communication between uAgents within the uAgents ecosystem.",
            path: "/references/uagents/uagents-protocols/exchange-protocol"
        }}
    />

</Grid2>
<Grid2>
    <GuideBox
        content={{
            title: "Storage",
            description: "uAgents storage is an important concept to understand how uAgents work.",
            path: "/references/uagents/uagents-protocols/storage"
        }}
    />

</Grid2>

## Fetch Network

### Ledger

<Grid2>

    <GuideBox
        content={{
            title: "Versions",
            description: "Explore the Ledger versions released over time.",
            path: "/references/fetch-network/ledger/versions"
        }}
    />

    <GuideBox
        content={{
            title: "Active networks",
            description: "Explore the list of active networks and their specifications.",
            path: "/references/fetch-network/ledger/active-networks"
        }}
    />

</Grid2>
<Grid2>

    <GuideBox
        content={{
            title: "Mainnet archives 🗄",
            description: "Explore the list of Mainnet network versions",
            path: "/references/fetch-network/ledger/mainnet-archive"
        }}
    />

    <GuideBox
        content={{
            title: "Validator security",
            description: "Discover the concept of security involving validators and their objectives.",
            path: "/references/fetch-network/ledger/security"
        }}
    />

</Grid2>

### CosmPy API

<div className="nx-text-fetch-content">
  Get started with all the technical information related to the CosmPy library.
</div>

<Grid2>

    <GuideBox
        content={{
            title: "Client",
            description: "General LedgerClient features.",
            path: "/references/fetch-network/cosmpy/client"
        }}
    />

    <GuideBox
        content={{
            title: "Contract",
            description: "Cosmwasm contract features to store, instantiate and execute messages.",
            path: "/references/fetch-network/cosmpy/contract"
        }}
    />

</Grid2>
<Grid2>

    <GuideBox
        content={{
            title: "Exceptions",
            description: "The `cosmpy.aerial.exceptions` module provides specialized exception classes for handling blockchain-related errors, including queries, broadcasts, and gas fees.",
            path: "/references/fetch-network/cosmpy/exceptions"
        }}
    />

    <GuideBox
        content={{
            title: "Ledger faucet API Interface",
            description: "The `cosmpy.aerial.faucet` module provides an interface to the Ledger faucet API, allowing users to interact with the faucet for a specific network configuration.",
            path: "/references/fetch-network/cosmpy/ledger-faucet-api-interface"
        }}
    />

</Grid2>
<Grid2>

    <GuideBox
        content={{
            title: "Network configurations",
            description: "The `cosmpy.aerial.config` module provides network configurations and handles errors related to network configurations.",
            path: "/references/fetch-network/cosmpy/network-configurations"
        }}
    />

    <GuideBox
        content={{
            title: "Parse the coins",
            description: "The `cosmpy.aerial.coins` module provides a method for parsing coins within a blockchain ecosystem.",
            path: "/references/fetch-network/cosmpy/parse-the-coins"
        }}
    />

</Grid2>
<Grid2>

    <GuideBox
        content={{
            title: "Parsing the URL",
            description: "The `cosmpy.aerial.urls` module provides classes and methods for parsing URLs within a blockchain ecosystem.",
            path: "/references/fetch-network/cosmpy/parsing-the-url"
        }}
    />

    <GuideBox
        content={{
            title: "Transaction",
            description: "The `cosmpy.aerial.tx` module provides classes and methods for handling transactions within a blockchain ecosystem.",
            path: "/references/fetch-network/cosmpy/transaction"
        }}
    />

</Grid2>
<Grid2>

    <GuideBox
        content={{
            title: "Transaction gas strategy",
            description: "The `cosmpy.aerial.gas` module provides gas strategies for handling transactions within a blockchain ecosystem.",
            path: "/references/fetch-network/cosmpy/transaction-gas-strategy"
        }}
    />

    <GuideBox
        content={{
            title: "Transaction helpers",
            description: "The `cosmpy.aerial.tx_helpers` module provides a set of utilities and objects to assist in transaction-related operations within a blockchain ecosystem.",
            path: "/references/fetch-network/cosmpy/transaction-helpers"
        }}
    />

</Grid2>
<Grid2>

    <GuideBox
        content={{
            title: "Wallet generation",
            description: "The `cosmpy.aerial.wallet` module provides functionalities for generating and managing wallets within a blockchain ecosystem.",
            path: "/references/fetch-network/cosmpy/wallet-generation"
        }}
    />

</Grid2>

### Indexer

<div className="nx-text-fetch-content">
Explore how to index and query blockchain data efficiently using the SubQuery indexer for the Fetch ledger.
</div>

<Grid2>

    <GuideBox
        content={{
            title: "Endpoints / Playground UIs",
            description: "The graphql API endpoints serve a playground UI to browsers for convenience.",
            path: "/references/fetch-network/indexer/endpoints"
        }}
    />

    <GuideBox
        content={{
            title: "Querying",
            description: "Understand how the GraphQL API works in conjunction with Postgraphile and its plugins for resolving different GraphQL requests.",
            path: "/references/fetch-network/indexer/querying"
        }}
    />

</Grid2>
<Grid2>

    <GuideBox
        content={{
            title: "Versioning",
            description: "Discover how to retrieve the versions of both the GraphQL API and the Indexer.",
            path: "/references/fetch-network/indexer/versioning"
        }}
    />

    <GuideBox
        content={{
            title: "Entities",
            description: "Entities are fundamental units of information that represent distinct objects or concepts, each with its own set of attributes.",
            path: "/references/fetch-network/indexer/entities"
        }}
    />

</Grid2>

<div className="nx-mb-8"></div>

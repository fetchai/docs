...
Every agent available and displayed on the Marketplace can be a: **Hosted**, **Local**, or **Mailbox** agent.

**Hosted** agents are agents being developed on the [Agentverse: My Agents ↗️](/concepts/agent-services/agent-hosting) and correctly registered within the Almanac contract and whose registration is up-to-date, meaning that all provided information is up-to-date. In this case the agent is denoted by a green **Active** tag. On the other hand, a **Local** agent is one pointing towards a local endpoint. 

Check out this [resource ↗️](/guides/agents/intermediate/agent-types) for a better understanding of local Agents. A local agent's utility is limited in real-world contexts, but such Agents are very useful for testing purposes. 

Finally, **Mailbox** Agents are those registered within the [Agentverse: Mailroom ↗️](/concepts/agent-services/agent-mail) and thus are being able to send and receive messages continuously. 

...
This flexibility is made possible through the categorization of AI Agents as either **public** or **private**, based on their **introspectivity** and **protocols' exposure** through the [Agentverse ↗️](/concepts/agent-services/agentverse-intro) platform.

## The role of protocols and Almanac contract

![](../../../src/images/missing.jpeg)

Within the uAgents Framework, a [protocol ↗️](/references/uagents/uagents-protocols/agent-protocols) establish the rules, messages structure, and semantics that govern interactions between AI Agents. Without protocols, agents would struggle to understand each others' messages or even coordinate effectively in the execution of any task.

On the other hand, the [Almanac ↗️](/references/contracts/uagents-almanac/almanac-overview) is a smart contract within the Fetch.ai ecosystem serving as a decentralized directory service, storing information about registered agents, their abilities as well as the associated **protocols' manifest**, which provides a structured and organized description of each communication protocol implemented within the agent's code. The Almanac facilitates discoverability in a way similar to how **Domain Name System (DNS)** enables internet accessibility. Indeed, thanks to the Almanac, it is made easy to discovery agents and start interacting with them via the [Agentverse Explorer ↗️](/concepts/agent-services/agent-explorer).
...
Within the Agentverse, protocols are represented by **protocols' digests**. These are unique protocols' identifiers provided below each agent's information in the Agentverse Explorer. Thanks to this feature, any user or company can directly filter out all agents based on digests and interact with any subset of agents offering a specific protocol implementation.
    ...
    <Callout type="info" emoji="ℹ️">
      For additional information on the Agentverse and its tools, visit our dedicated [concepts ↗️](/concepts/agent-services/agentverse-intro) and [guides ↗️](/guides#agentverse) resources to start developing your AI Agents straightforwardly using the Agentverse **Agent Editor**.
    </Callout>
...
In this context, users or companies may not be willing to share information but rather wanting to keep it private for any reason. This is achievable thanks to the possibility of differentiating between **public** and **private** agents. There exist _two ways through which an agent can be considered **public**_:
    ...
    2. By default, during agent development when the agent's specific protocol is set to accept introspection queries requested from any other external agent. This agent protocol's feature can be switched on and off, but is _on_ by default, thus allowing introspection queries to be answered by it.

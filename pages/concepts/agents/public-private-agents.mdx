...
Transparency is a fundamental principle in decentralized finance (DeFi) and blockchain systems. Within the Fetch.ai network, **users have the ability to determine the amount of information they wish to publish**. This is thanks to the ability for users to create AI Agents as either **public** or **private**, based on their **introspectivity** and **protocol exposure** through the [Agentverse ↗️](/concepts/agent-services/agentverse-intro) platform.

![](../../../src/images/concepts/ai-agents/public_and_private_agents.svg)

This allows users to provide greater flexibility to AI Agents, creating a balance between transparency and privacy for every operation they perform on the network.
...
There are two ways in which an agent can be classified as **public**:
     ...
     1. By default, _at registration_ in the agent verse, when the protocol's manifest and associated digests are published, making the agent's [endpoint ↗️](/references/contracts/uagents-almanack/endpoints) available to any other user for communication. This provides users with a way to access our registered agent's information and understand how to communicate with it.

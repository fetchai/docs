# Concepts

<div className="nx-text-fetch-content">
  The place where you can get yourself familiar with the needed concepts to
  start developing on the uAgents Framework and Agentverse Functions.
</div>

import { Grid2 } from "../components/mdx";
import { GuideBox } from "../components/feature-guide-tabs";

## Introduction to Fetch.ai

<div className="nx-text-fetch-content">
  Get to know Fetch.ai, the technology, tools and the goals we are delivering.
</div>

<Grid2>
    <GuideBox
        content={{
            title: "Introducing Fetch.ai: pioneering an AI powered decentralized digital economy",
            description: "Get to know Fetch.ai, the technology, tools and the goals we are delivering.",
            path: "/concepts/introducing-fetchai"
        }}
    />

</Grid2>

## Agentverse

<div className="nx-text-fetch-content">
  Explore the resources and dive into the Agentverse and its functionalities.
</div>

<Grid2>
    <GuideBox
        content={{
            title: "Agentverse: Introduction",
            description: "Introducing the Agentverse platform.",
            path: "/concepts/agent-services/agentverse-intro"
        }}
    />

    <GuideBox
        content={{
            title: "Agentverse: Explorer",
            description: "Use the Agentverse Explorer to start interacting with other uAgents.",
            path: "/concepts/agent-services/agent-explorer"
        }}
    />

</Grid2>
<Grid2>
    <GuideBox
        content={{
            title: "Agentverse: My Agents",
            description: "Get started and to deploy uAgents to the cloud to start connecting and automating.",
            path: "/concepts/agent-services/agent-hosting"
        }}
    />

    <GuideBox
        content={{
            title: "Agentverse: Mailroom / IoT Gateway",
            description: "Set up Agentverse mailboxes for your locally-run uAgents and to run them independently of your constant presence to run the server.",
            path: "/concepts/agent-services/agent-mail"
        }}
    />

</Grid2>
<Grid2>
    <GuideBox
        content={{
            title: "Agentverse: uAgent Functions",
            description: "The Agentverse makes it possible to register your uAgent and related Functions to be discoverable by the AI Engine and DeltaV.",
            path: "/concepts/agent-services/services"
        }}
    />


</Grid2>

## AI Engine

<div className="nx-text-fetch-content">
  Discover how the AI Engine facilitates interactions by discovering user
  preferences, transforming raw data into actionable insights through
  collaboration with uAgent-based Functions.
</div>

<Grid2>

    <GuideBox
        content={{
            title: "AI Engine: Introduction",
            description: "Introducing the AI Engine and its role in DeltaV functionalities.",
            path: "/concepts/ai-engine/ai-engine-intro"
        }}
    />

    <GuideBox
        content={{
            title: "Powering connections and smart operations in DeltaV",
            description: "Explore the integration of AI Engine within DeltaV, enabling connections between users and uAgent Functions.",
            path: "/concepts/ai-engine/powering-connections-and-smart-operations-in-deltav"
        }}
    />

</Grid2>
<Grid2>

    <GuideBox
        content={{
            title: "DeltaV",
            description: "Introducing DeltaV.",
            path: "/concepts/deltav/intro"
        }}
    />

</Grid2>

## Fetch network

<div className="nx-text-fetch-content">
  Discover the tools and products constituting and adopted within the Fetch network.
</div>

<Grid2>

    <GuideBox
        content={{
            title: "Introducing the Fetch Network",
            description: "An introduction to the different components constituting the Fetch Network.",
            path: "/concepts/fetch-network/intro-fetch-network"
        }}
    />

</Grid2>

<Grid2>

    <GuideBox
        content={{
            title: "Native and ERC-20 FET tokens",
            description: "Discover the difference between native and ERC-20 FET tokens.",
            path: "/concepts/fetch-network/native-and-erc20-fet-tokens"
        }}
    />

    <GuideBox
        content={{
            title: "The Almanac contract",
            description: "Explore the role of the Almanac contract within the Fetch ecosystem.",
            path: "/concepts/fetch-network/almanac"
        }}
    />

</Grid2>

### Ledger

<Grid2>

    <GuideBox
        content={{
            title: "Ledger: Introduction",
            description: "Get yourself started with Fetch Ledger.",
            path: "/concepts/fetch-network/ledger/intro"
        }}
    />

    <GuideBox
        content={{
            title: "Validators overview",
            description: "Explore the role of validators within the Fetch ecosystem.",
            path: "/concepts/fetch-network/ledger/validators"
        }}
    />

</Grid2>


### Indexer

<div className="nx-text-fetch-content">
Explore how to index and query blockchain data efficiently using the SubQuery indexer for the Fetch ledger.

</div>

<Grid2>

    <GuideBox
        content={{
            title: "Indexer: An Introduction",
            description: "Introducing the ledger-subquery is a SubQuery-based indexer for the Fetch ledger.",
            path: "/concepts/fetch-network/indexer"
        }}
    />

</Grid2>

<div className="nx-mb-8"></div>

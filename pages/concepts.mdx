...
When an Agent comes online for it to be accessible to the wider network it must register to the Almanac - currently this is done for you in a test network (meaning no payments are made). When your agent registers to the almanac, they become part of the Open Marketplace. AI Engine consumes events that come from all of the main agentverse services (hosting, mailbox, almanac) to build its search index. For an agent to be accessible by AI-Enging they need to register their agent function metadata to the Agentverse. The Fetch Network offers a layer of truth and trust by inherently being open.

...
Protocols are used to facilitate communication and interaction between uAgents in the Framework.

    <Callout type="info" emoji="ℹ️">
      Indeed, any uAgent including the same protocol will be able to communicate with each other.
    </Callout>

A `protocol` is built similar to an uAgent, but it has no identity and cannot be run. Protocols only contains the message types and handlers that define some components of an uAgent's functionality.
...
4. We can then import our booking protocol from into the script we create for our uAgent, in the following way:

   ```py copy
   from protocols.book import book_proto
   ```

5. If your uAgent is called `restaurant` you can include the protocol in this way:

   ```py copy
   restaurant.include(book_proto)
   ```

For a better understanding of these concepts, consider having a look at the [uAgents storage ↗️](/references/uagents/uagents-protocols/storage) and [Exchange protocol ↗️](/references/uagents/uagents-protocols/exchange-protocol) resources and consider going through the extensive [How to book a table at a restaurant using uAgents ↗️](/examples/table-booking-demo) guide in the [uAgents guides ↗️](/guides#ai-agents) section. Also, checkout the [uAgents: broadcast ↗️](/examples/broadcast) for an additional implementation of protocols in uAgents communication.

...
This guide offers details on utilizing the AI engine's chat API to interact with [AI
Engine ↗️](/concepts/ai-engine/ai-engine-intro) and create [DeltaV ↗️](https://deltav.agentverse.ai) like GUI of own. We will demonstrate a python script that interacts with AI engine via the chat API, handling session IDs, sending messages, managing responses, and processing user requests and selections.
...
5. `send_user_choice_as_uuid` function:
    ...
    This function responds makes request to AI_engine with UUID of first subtask from given recommendations. The payload parameter are explained below:
        ...
        - `first_subtask` - UUID for the secondary task.
...
- Taking user input from the user for function they would like to assemble/Objective they want to accomplish:
...
```
    ...
    - Handling `agent_message` type response:
        ...
        2. If the response is task is completed the session is stopped.
...
- Handling `agent_info`, `stop` and `unknown` Type Responses:
        ...
        4. If the response type is not known it prints unhandled response type.
...
Running this app on the terminal:
    ...
    8. Once the final response is received session will be ended and script will be killed.

Please refer below for expected output of `chatAPI.py` script:

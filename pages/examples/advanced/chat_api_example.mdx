...
This guide offers details on utilizing the AI engine's [chat API ↗️](../apis/ai-engine/chat) to interact with AI engine and create [DeltaV ↗️](../guides/deltav/deltav-chat-interface) like GUI. We will demonstrate a python script that interacts with AI engine via the chat API, handling session IDs, sending messages, managing responses, and processing user requests and selections.
...
Running this app on the terminal:
    ...
    2. Open [Agentverse ↗️](https://agentverse.ai/profile/api-keys) and [generate API keys](chat_api_example#steps-to-get-api-tokens).

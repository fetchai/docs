...
This guide offers details on utilizing the AI engine's [chat API ↗️](../apis/ai-engine/chat) to interact with AI engine and create [DeltaV ↗️](htttps://deltav.agentverse.ai) like GUI. We will demonstrate a python script that interacts with AI engine via the chat API, handling session IDs, sending messages, managing responses, and processing user requests and selections.

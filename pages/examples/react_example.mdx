...
This example shows how to build a react application integrated with a flask backend, using various agents to perform tasks such as fetching news, scrapping webpage data and also getting the sentiment of news using hugging face finbert model of [HF infrence API](https://huggingface.co/docs/api-inference/en/index).

### Supporting Documents

    - [How to use on_query decorator](../guides/agents/on-query)
    - [How to create an agent](../guides/agents/create-a-uagent)
    - [Registering in the Almanac Contract](../guides/agents/register-in-almanac)
    - [Almanac contract overview](/references/contracts/uagents-almanac/almanac-overview)
...
- Define routes for fetching news_urls, scrapping news content and get finber sentiments.
...
# Define an asynchronous endpoint to analyze sentiment for a given company
...
2. Agents
    ...
    - **News and URL agent:** Fetchs news articles titles and url for them as well.
...
async def query_handler(ctx: Context, sender: str, msg: NewsRequest):
    try:
        ...
        if symbol  != None: #if comapny symbol fetch successfully getting news using ticker symbol else using the company name itself.
...
# On_query handler for processinf sentiment request
...
## Setup and Running the applicatio

...
The example showcases the use of agent protocols to facilitate communication and interaction between the two agents involved in this operation. In few words, a `protocol` is built similarly to any agent, but it has no identity and cannot be run on its own. Protocols only contains the message types and handlers that define some components of an agent's functionality. For this example, we need two protocols:

    - **Querying protocol**: it defines the functionalities for querying table availability in the restaurant booking system.
    - **Booking protocol**: it defines functionalities for booking a table at the restaurant.

Both protocols are then included within the agents' code. These protocols' scripts work together to allow customers to query the restaurant about available tables and attempt to book a table based on their needs. The `query.py` script helps find suitable tables, while `book.py` handles the actual reservation process.
...
- [Agent Storage ↗️](/guides/agents/intermediate/storage-function)
...
The below script defines the `query_proto` protocol, a key component in this restaurant booking system. This protocol allows customers to inquire about tables that meet their needs. It defines the following **Data Models**:
...
This below script delves into the `book_proto` table protocol, which is another critical component in this restaurant booking system. The `book` protocol empowers customers to attempt booking a specific table based on availability. It defines the following **Data Models**:
...
Let's now define the agents involved in this table booking process. This example relies on two agents to manage the restaurant booking process: the `restaurant` agent and the `user` agent. Both agents work collaboratively to ensure a seamless experience for both diners and the restaurant staff. Indeed, the example flow is as below:
...
Outputs are showed in different terminals depending on the agent.

a. `restaurant` agent output:

    ```
    INFO:     [restaurant]: Almanac registration is up to date!

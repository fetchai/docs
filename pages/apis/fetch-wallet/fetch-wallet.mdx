...
## Detect Fetch Browser Wallet

<Row>
    <Col>
        You can determine whether Fetch Wallet is installed on the user's device by checking `window.fetchBrowserWallet`.

        If `window.fetchBrowserWallet` returns `undefined` after the document loads, then the Fetch Browser Wallet is not installed.
...
<Row>
    <Col>
    The `@fetchai/wallet-types` package has the type definition related to Fetch Browser Wallet.
    ...
    Then, you can add the `@fetchai/wallet-types` window to a global window object and register the Fetch Browser Wallet related types.
...
<Row>
    ...
    To assist dApp creators, we support Keplr APIs, allowing them to use their existing logic with the Fetch wallet. This support facilitates a gradual transition to more advanced APIs of the fetch browser wallet.
...
<Row>
    <Col>
    To access Fetch Wallet APIs, you can simply refer to the `wallet` property of the `FetchBrowserWallet` window object.
...
<Row>
    <Col>
    Provides the status of the wallet, indicating whether it is **`NOTLOADED`**, **`EMPTY`**, **`LOCKED`**, or **`UNLOCKED`**. Description for each status can be found [here ↗️](/apis/fetch-wallet#walletstatus).
...
<Row>
    ...
    **Returns**
    ...
    Further reference documentation is available here: [NetworkConfig ↗️](/apis/fetch-wallet#networkconfig)
...
<Row>
    ...
    **Params** (required)
    ...
    [NetworkConfig ↗️](/apis/fetch-wallet#networkconfig)
...
<Row>
    ...
    **Returns**
    ...
    Further reference documentation is available here: [Account ↗️](/apis/fetch-wallet#account)
...
<Row>
    ...
    **Returns**
    ...
    Further reference documentation is available here: [AddressBookEntry ↗️](/apis/fetch-wallet#addressbookentry)
...
<Row>
    ...
    **Params**
    ...
    [AddressBookEntry ↗️](/apis/fetch-wallet#addressbookentry)
...
<Row>
    ...
    **Params**
    ...
    3. **`StdSignDoc`** *(object)*: The sign document containing transaction details.
        ...
        Further reference documentation is available here: [StdSignDoc ↗️](/apis/fetch-wallet#stdsigndoc)
    ...
    **Returns**
    ...
    Further reference documentation is available here: [AminoSignResponse ↗️](/apis/fetch-wallet#aminosignresponse)
...
<Row>
    <Col>
    Sign transactions directly without using Amino encoding. Signs Proto-encoded `StdSignDoc` ([StdSignDoc ↗️](/apis/fetch-wallet#aminosignresponse))
    ...
    **Returns**
    ...
    Further reference documentation is available here: [DirectSignResponse ↗️](/apis/fetch-wallet#directsignresponse)
...
<Row>
    ...
    **Returns**
    ...
    Further reference documentation is available here: [StdSignature ↗️](/apis/fetch-wallet#stdsignature)
...
<Row>
    ...
    **Params**
    ...
    2. `type` *([EthSignType ↗️](/apis/fetch-wallet#ethsigntype))*: The type of Ethereum signature.
...
<Row>
    ...
    - **`address`** *(string)*
        ...
        The representation is chain specific. For example in the case of the Fetch chain it should be bech32 encoded andshould have the prefix `fetch`.
...
<Row>
    <Col>
    - **`signed`** *([StdSignDoc↗️](/apis/fetch-wallet#stdsigndoc))*

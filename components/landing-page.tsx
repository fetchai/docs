...
function LandingPage() {
    ...
    const GuideBox = ({
        ...
        const backgroundTransitions: string[] = ["phaseOne", "phaseTwo", "phaseThree"]

        const guideBox = "nx-relative nx-p-8 nx-rounded " + backgroundTransitions[v] + " "  + height
        ...
        <section className={styles.page}>
            ...
            <section className="nx-mt-12">
                ...
                <div className="nx-grid nx-grid-cols-3 sm:nx-grid-cols-2 md:nx-grid-cols-3 nx-gap-4">
                    {startingGuides.map((guide, index) => {
                        return <GuideBox key={index} guide={guide} v={index} height="nx-h-96"/>;
                    })}
                </div>
            </section>
            ...
            <section className="nx-mt-60">
                ...
                <div className={styles.startGuides}>
                    <div
                        className="nx-grid nx-grid-cols-2 sm:nx-grid-cols-2 md:nx-grid-cols-3 lg:nx-grid-cols-4 nx-gap-4">
                        {beginnerGuides.map((guide, index) => {
                            return <GuideBox key={index} guide={guide} v={1} height="nx-h-8"/>;
                        })}
                    </div>
                </div>
            </section>

...
# Word pattern to extract valid words (excluding numbers, CSS-like values, and words with underscores)
word_pattern = re.compile(r"\b[a-zA-Z]+(?:'[a-zA-Z]+)?\b")

# Function to extract text from MDX files while ignoring specific components
def extract_text_from_mdx(file_path):
    with open(file_path, 'r') as file:
        content = file.read()

    # Remove import statements, paths, GuideBox components, and JSX-like tags
    content = import_pattern.sub('', content)
    content = path_pattern.sub('', content)
    content = guidebox_pattern.sub('', content)
    content = jsx_like_tags_pattern.sub('', content)
    ...
    # Extract text while skipping code blocks and inline code
    text = []

    def traverse(node):
        if node.type in {'fence', 'code_inline'}:  # Skip fenced code blocks and inline code
            return
        if node.type == 'text':
            text.append(node.content)
        for child in node.children or []:
...
# Function to check for spelling errors
def check_spelling(text):
    # Extract words using the word pattern
    words = word_pattern.findall(text)

    # Filter out custom words
    words_to_check = [word.lower() for word in words if word.lower() not in custom_words]
